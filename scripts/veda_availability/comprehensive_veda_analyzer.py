...
class VEDAAnalyzer:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/collections") as response:
                    if response.status == 200:
                        ...
                        logger.error(
                            f"[FAIL] Failed to fetch collections: HTTP {response.status}"
                        )
                        return []
    ...
    async def analyze_collection(
        self, collection_id: str, session: aiohttp.ClientSession
    ) -> Dict[str, Any]:
        ...
        for strategy_name, strategy_config in self.test_strategies.items():
            try:
                # Build query
                query = {"collections": [collection_id], "limit": 5}
                query.update(strategy_config["params"])

                logger.info(
                    f"  [CHART] Testing strategy '{strategy_name}' for {collection_id}"
                )
                ...
                async with session.post(self.search_url, json=query) as response:
                    if response.status == 200:
                        ...
                        if features and not collection_result["best_strategy"]:
                            ...
                            collection_result["asset_types"] = strategy_result.get(
                                "asset_types", []
                            )

                        logger.info(
                            f"    [OK] Strategy '{strategy_name}': {len(features)} features found"
                        )
                        ...
                        logger.warning(
                            f"    [FAIL] Strategy '{strategy_name}': HTTP {response.status}"
                        )
        ...
        if collection_result["working_strategies"]:
            success_rate = len(collection_result["working_strategies"]) / len(
                self.test_strategies
            )
            if success_rate >= 0.8:
        ...
        logger.info(
            f"[UP] Collection {collection_id}: {collection_result['status']} ({len(collection_result['working_strategies'])}/{len(self.test_strategies)} strategies working)"
        )
    ...
    async def analyze_all_collections(self):
        ...
        logger.info(
            f"[DOCS] Collected metadata for {len(self.collections_metadata)} collections"
        )
...
"""
        ...
        working_collections.sort(
            key=lambda x: (x["status"] == "excellent", x["working_strategies"]),
            reverse=True,
        )
        ...
        if problematic_collections:
            report += (
                f"## Problematic Collections ({len(problematic_collections)} total)\n\n"
            )
    ...
    def save_results(self, output_dir: str = "veda_analysis_results"):
        ...
        working_profiles = {}
        for collection_id, result in self.results.items():
            if result.get("status") in ["excellent", "good", "limited"] and result.get(
                "working_strategies"
            ):
    ...
    def _categorize_collection(self, collection_id: str, metadata: Dict) -> str:
        ...
        if any(
            keyword in collection_lower or keyword in title_lower
            for keyword in ["fire", "burn", "thermal", "barc", "modis-14", "viirs"]
        ):
            return "Fire Detection"
        elif any(
            keyword in collection_lower or keyword in title_lower
            for keyword in ["era5", "climate", "temperature", "pressure", "weather"]
        ):
            ...
            keyword in collection_lower or keyword in title_lower
            for keyword in ["ocean", "sea", "marine", "sst", "chlorophyll"]
        ):
            return "Ocean/Marine"
        elif any(
            keyword in collection_lower or keyword in title_lower
            for keyword in ["urban", "population", "city", "infrastructure"]
        ):
            return "Urban/Infrastructure"
        elif any(
            keyword in collection_lower or keyword in title_lower
            for keyword in ["agriculture", "crop", "farming", "agricultural"]
        ):
            return "Agriculture"
        elif any(
            keyword in collection_lower or keyword in title_lower
            for keyword in ["disaster", "hurricane", "flood", "earthquake"]
        ):
