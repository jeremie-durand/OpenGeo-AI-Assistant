...
class UnifiedPCExtractor:
    ...
    def extract_all(self):
        ...
        for folder in sorted(dataset_folders):
            ...
            if subdirs and not (collection_folder / "config.json").exists():
                ...
                print(
                    f"    Found multi-collection folder: {folder.name} ({len(subdirs)} collections)"
                )
        ...
        if template_file.exists():
            with open(template_file, encoding="utf-8") as f:
                ...
                if "title" in template:
                    ...
                    for asset_name, asset_info in template.get(
                        "item_assets", {}
                    ).items():
                        if "title" in asset_info:
                            metadata["title"] = (
                                f"{collection_id} - {asset_info['title']}"
                            )
                            break
    ...
    def _classify_data_type(
        ...
        # Temperature/Thermal
        if (
            any(x in cid_lower for x in ["11a1", "11a2", "21a", "lst", "temperature"])
            or "temperature" in desc_lower
        ):
            return "thermal"

        # SAR
        if (
            "sentinel-1" in cid_lower
            or "sar" in cid_lower
            or "alos-palsar" in cid_lower
        ):
            return "sar"

        # Elevation/DEM
        if (
            any(x in cid_lower for x in ["dem", "elevation", "nasadem", "3dep"])
            or "elevation" in desc_lower
        ):
            return "elevation"

        # Surface reflectance (MODIS 09, 43)
        if (
            any(x in cid_lower for x in ["09a1", "09q1", "43a4"])
            or "reflectance" in desc_lower
        ):
        ...
        # Climate
        if (
            any(x in cid_lower for x in ["era5", "climate", "terraclimate"])
            or "climate" in desc_lower
        ):
            return "climate"
    ...
    def _generate_description(
        self, collection_id: str, metadata: Dict, data_type: str
    ) -> str:
        ...
        if "io-lulc" in collection_id:
            year = (
                ...
                if collection_id.split("-")[-1].isdigit()
                else "annual"
            )
        ...
        return type_descriptions.get(
            data_type, f"{title} geospatial dataset from Microsoft Planetary Computer."
        )
    ...
    def export_unified_json(self):
        ...
        print("      - Rendering configs, metadata, classification")
