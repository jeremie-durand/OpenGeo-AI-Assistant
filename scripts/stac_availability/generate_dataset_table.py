...
def fetch_description_from_pc_api(collection_id: str) -> str:
    ...
    try:
        ...
        print(
            f"  [WARN] Could not fetch description from PC API for {collection_id}: {e}"
        )
        return None
...
def extract_description(description_file: Path, collection_id: str = None) -> str:
    ...
    if collection_id:
        print(
            f"  [SIGNAL] Fetching description from Planetary Computer API for {collection_id}..."
        )
...
def generate_markdown_table(datasets: List[Dict[str, Any]]) -> str:
    ...
    for dataset in datasets:
        ...
        md += (
            f"| {dataset['description'][:100]}... "
            if len(dataset["description"]) > 100
            else f"| {dataset['description']} "
        )
