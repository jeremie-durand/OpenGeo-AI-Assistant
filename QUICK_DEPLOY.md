...
> **Why manual?** Creating app registrations requires Microsoft Graph API permissions (`Application.ReadWrite.All`) — these are directory-level permissions separate from Azure RBAC. A standard deployment service principal (Contributor + User Access Administrator) doesn't have them. This is a deliberate security boundary in Microsoft Entra ID.
...
```powershell
# Trigger deployment — fully private by default; auth enabled if AUTH_CLIENT_ID is set (Step 8.3)
