        ...
        if matched_tool and not is_overview_query and not is_comparison_query:
            ...
            try:
                ...
                if is_trend_query:
                    ...
                    logger.info(f"[STORM] [{request_id}] FAST PATH TREND: Parallel {tool_name} for years {trend_years}")
                    
                    trend_tasks = [
                        loop.run_in_executor(None, tool_fn, latitude, longitude, scenario, yr)
                    ...
                    for i, (yr, raw) in enumerate(zip(trend_years, trend_results_raw)):
                        if isinstance(raw, Exception):
                            logger.warning(f"[STORM] [{request_id}] Trend year {yr} failed: {raw}")
                            continue
                        td = json.loads(raw)
                        if "error" not in td:
                    ...
                    fast_elapsed = time.time() - agent_start
                    logger.info(f"[STORM] [{request_id}] FAST PATH TREND: Data for {len(all_data_parts)}/{len(trend_years)} years in {fast_elapsed:.1f}s")
                    ...
                    if all_data_parts:
                        ...
                        formatted_response = fmt_resp.choices[0].message.content
                        total_elapsed = time.time() - agent_start
                        logger.info(f"[STORM] [{request_id}] FAST PATH TREND COMPLETE in {total_elapsed:.1f}s")
