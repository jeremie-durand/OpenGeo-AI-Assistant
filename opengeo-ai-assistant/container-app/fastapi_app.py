...
# ============================================================================
pipeline_traces: TTLCache = TTLCache(
    maxsize=500, ttl=1800
)  # 30-min TTL, max 500 sessions
...
SKIP_DEDUP_COLLECTIONS = [
    "modis-14A1-061",
    "modis-14A2-061",  # Fire detection
    "sentinel-1-rtc",
    "sentinel-1-grd",  # SAR (different polarizations)
]


def should_deduplicate_tiles(
    collections: List[str], original_query: str = None
) -> bool:
    """
    ...
    is_optical = any(
        any(opt in coll.lower() for opt in OPTICAL_COLLECTIONS) for coll in collections
    )
    ...
    if original_query:
        ...
        if has_temporal_intent:
            logging.info(
                f"[SYNC] Dedup: Skipping - temporal query detected: '{original_query[:50]}...'"
            )
            return False
...
def deduplicate_tiles_by_grid(
    features: List[Dict[str, Any]], original_query: str = None
) -> List[Dict[str, Any]]:
    """
    ...
    for feature in features:
        ...
        if grid_id not in seen_grids:
            ...
            logging.debug(
                f"[SYNC] Dedup: Skipping duplicate grid {grid_id} (feature: {feature.get('id', 'unknown')[:50]})"
            )

    if len(deduplicated) < original_count:
        logging.info(
            f"[TARGET] TILE DEDUPLICATION: {original_count} -> {len(deduplicated)} tiles (removed {original_count - len(deduplicated)} duplicates)"
        )
        logging.info(
            f"[TARGET] Unique grid cells: {list(seen_grids)[:10]}{'...' if len(seen_grids) > 10 else ''}"
        )
...
try:
    from pc_tasks_config_loader import get_all_collection_ids, get_collection_metadata
    from pc_tasks_config_loader import get_pc_rendering_config as get_rendering_config
    ...
    logging.info(
        f"[OK] PC metadata loaded: {total_collections} collections from PC repository"
    )
    ...
    # Fallback stubs
    def load_pc_metadata():
        return {"categories": []}

    def get_collection_metadata(cid):
        return None

    def get_rendering_config(cid):
        return None
...
def check_collection_coverage(collection_id: str, bbox: list) -> dict:
    ...
    return {
        "covered": False,
        "message": f"Collection {collection_id} not found in PC metadata",
    }
...
if ENABLE_AUTH:
    ...
    logger.warning(
        "[AUTH] ENABLE_AUTH is false — all routes are open (no authentication checks)"
    )
...
async def root() -> dict:
    ...
    return {
        "message": "OpenGeo AI Assistant API is running",
        "status": "ok",
        "version": "1.0.0",
    }
...
async def execute_direct_stac_search(
    ...
    try:
        ...
        logger.info(
            f"[SEARCH] STAC SEARCH: {stac_endpoint} | collections={stac_query.get('collections', [])} | bbox={stac_query.get('bbox', 'NONE')} | datetime={stac_query.get('datetime', 'NONE')} | limit={stac_query.get('limit', 'default')}"
        )
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(stac_url, json=stac_query) as response:
                ...
                if response.status == 200:
                    ...
                    if features:
                        collections_returned = list(
                            set(f.get("collection", "?") for f in features)
                        )
                        logger.info(
                            f"[OK] STAC RESULTS: {len(features)} items from {collections_returned}"
                        )
                    else:
                        logger.warning("[WARN] STAC RESULTS: 0 items returned")
                    ...
                    if len(features) == 0:
                        ...
                        if collections and bbox:
                            ...
                            for collection_id in collections:
                                coverage_check = check_collection_coverage(
                                    collection_id, bbox
                                )
                                if not coverage_check["covered"]:
                                    coverage_info = {
                                        "message": coverage_check["message"],
                                        "alternatives": coverage_check.get(
                                            "alternatives", []
                                        ),
                                    }
                                    logger.info(
                                        f"ℹ️ No results - likely due to coverage: {coverage_info['message']}"
                                    )
                                    break
                    ...
                    if bbox and len(bbox) == 4:
                        ...
                        tiles_for_coverage = int(
                            tiles_wide * tiles_tall * 1.5
                        )  # 50% overlap margin

                        if area_degrees > 25:  # Country-scale (e.g., Greece ~71 sq deg)
                            max_tiles = min(
                                100, max(50, tiles_for_coverage)
                            )  # 50-100 tiles for countries
                            logger.info(
                                f"[GLOBE] Country-scale area ({area_degrees:.1f} sq deg) -> max_tiles={max_tiles}"
                            )
                        elif (
                            area_degrees > 5
                        ):  # Large region (e.g., California, large state)
                            max_tiles = min(
                                60, max(30, tiles_for_coverage)
                            )  # 30-60 tiles
                            logger.info(
                                f"[MAP] Large region ({area_degrees:.1f} sq deg) -> max_tiles={max_tiles}"
                            )
                        elif area_degrees > 1.0:  # Medium region (multi-city area)
                            max_tiles = min(
                                30, max(15, tiles_for_coverage)
                            )  # 15-30 tiles
                    ...
                    selected_features = deduplicate_tiles_by_grid(
                        selected_features, original_query
                    )

                    logger.info(
                        f"[TARGET] TILE PIPELINE: {len(features)} raw -> {len(selected_features)} final tiles (max={max_tiles})"
                    )
                    ...
                    logger.error(
                        f"[FAIL] STAC API error {response.status}: {error_text}"
                    )
                    return {
                        "success": False,
...
def detect_collections(query: str) -> List[str]:
    ...
    if any(term in query for term in ["fire", "wildfire", "burn", "thermal", "heat"]):
        ...
        detected_collections.extend(
            ["landsat-c2-l2", "sentinel-2-l2a", "modis-14A1-061"]
        )

    elif any(
        term in query
        for term in ["elevation", "dem", "topography", "terrain", "height"]
    ):
        ...
        detected_collections.extend(
            ["sentinel-2-l2a", "landsat-c2-l2", "modis-13A1-061"]
        )
...
def generate_contextual_empty_response(
    query: str, collections: List[str], diagnostics: Dict[str, Any]
) -> str:
    """
    ...
    if raw_count == 0:
        response_parts.append(
            f"I searched for satellite data related to '{query}', but didn't find any available imagery matching your criteria."
        )
        ...
        if "eo:cloud_cover" in str(cloud_filter):
            ...
            if isinstance(cloud_filter, dict):
                try:
                    ...
                    logger.warning(
                        "Failed to extract cloud cover threshold from filter: %s", e
                    )

            if cloud_threshold:
                suggestions.append(
                    f"**Relax cloud cover filter**: Currently requiring <{cloud_threshold}% clouds. Try allowing up to {cloud_threshold + 20}% for more options."
                )
        ...
        if collections:
            ...
            suggestions.append(
                f"**Verify data availability**: Searched {collection_names} - confirm this data exists for your location and time period."
            )
        ...
        if any(
            term in query_lower
            for term in ["ocean", "sea", "water", "pacific", "atlantic"]
        ):
            suggestions.append(
                "**Check location**: Some collections (like NAIP) only cover land areas, not oceans."
            )
    ...
    # SCENARIO 2: STAC returned results, but spatial filter removed them
        ...
        response_parts.append("\n**This usually means:**")
        response_parts.append(
            "- The imagery tiles only partially overlap your location (less than 10% coverage)"
        )
        response_parts.append(
            "- Your search area might be at the edge of the satellite's coverage zone"
        )
        response_parts.append(
    ...
    # SCENARIO 4: Unknown failure
        ...
        response_parts.append(
            f"I couldn't find satellite data matching your query for '{query}'."
        )
        response_parts.append(
...
async def try_alternative_queries(
    ...
    if original_filters["cloud_cover"] and original_filters["cloud_cover"] < 50:
        for cloud_threshold in [30, 50]:
            ...
            logger.info(
                f"[WEATHER] Trying alternative with cloud cover <{cloud_threshold}%..."
            )
                ...
                if alt_response.get("success"):
                    ...
                    if alt_features:
                        ...
                        if translator and requested_bbox:
                            filtered_results = (
                                translator._filter_stac_results_by_spatial_overlap(
                        ...
                        if alt_features:
                            ...
                            if requested_bbox and len(requested_bbox) == 4:
                                fb_area = (requested_bbox[2] - requested_bbox[0]) * (
                                    requested_bbox[3] - requested_bbox[1]
                                )
                                if fb_area > 25:
                            ...
                            if selected_features:
                                logger.info(
                                    f"[OK] Found {len(selected_features)} results with cloud cover <{cloud_threshold}%"
                                )
                                return {
                                    "success": True,
                                    "features": selected_features,
    ...
    logger.info(
        f"[DATE] RELAXATION 2: Checking date range expansion for datetime='{datetime_str}'"
    )
    if datetime_str and "/" in datetime_str:
        try:
            from datetime import datetime as dt
            ...
            for expand_days in [30, 60, 90]:
                ...
                logger.info(
                    f"[DATE] Trying alternative with {expand_days}-day date range..."
                )
                ...
                logger.info(
                    f"[DATE] Alt query collections: {alt_query.get('collections')}, bbox: {alt_query.get('bbox')}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"[DATE] Alt response success: {alt_response.get('success')}, features: {len(alt_response.get('results', {}).get('features', []))}"
                    )

                    if alt_response.get("success"):
                        alt_features = alt_response.get("results", {}).get(
                            "features", []
                        )
                        ...
                        if alt_features:
                            ...
                            if alt_features:
                                ...
                                if requested_bbox and len(requested_bbox) == 4:
                                    fb_area = (
                                        requested_bbox[2] - requested_bbox[0]
                                    ) * (requested_bbox[3] - requested_bbox[1])
                                    if fb_area > 25:
                                        fallback_max_tiles = 100
                                    elif fb_area > 5:
                                ...
                                if selected_features:
                                    logger.info(
                                        f"[OK] Found {len(selected_features)} results with {expand_days}-day range"
                                    )
                                    return {
                                        "success": True,
                                        "features": selected_features,
                    ...
                    logger.warning(
                        f"[WARN] Date expansion failed for {expand_days} days: {type(e).__name__}: {e}"
                    )
                    import traceback
    ...
    if any("sentinel" in c.lower() for c in original_collections) and not any(
        "hls" in c.lower() for c in original_collections
    ):
        alternative_collection_sets.append(
            {"collections": ["landsat-c2-l2"], "name": "Landsat"}
        )

    for alt_collections in alternative_collection_sets:
        logger.info(
            f"[SAT] Trying alternative collections: {alt_collections['name']}..."
        )
            ...
            if alt_response.get("success"):
                ...
                if alt_features:
                    ...
                    if alt_features:
                        ...
                        if selected_features:
                            logger.info(
                                f"[OK] Found {len(selected_features)} results with {alt_collections['name']}"
                            )
                            return {
                                "success": True,
                                "features": selected_features,
...
def generate_fallback_response(
    query: str, features: List[Dict], collections: List[str]
) -> str:
    """
    ...
    try:
        ...
        if feature_count == 0:
            response_parts.append(
                f"I searched for satellite data related to '{query}', but didn't find any available imagery for the specified area and time period."
            )
            response_parts.append(
                "Try adjusting your search area or time range, or check if the location name is spelled correctly."
            )
        elif feature_count == 1:
            response_parts.append(
                f"Found 1 satellite image matching your query for '{query}'."
            )
        else:
            response_parts.append(
                f"Found {feature_count} satellite images matching your query for '{query}'."
            )
            ...
            try:
                ...
                if dates:
                    ...
                    if earliest == latest:
                        ...
                        response_parts.append(
                            f"The imagery spans from {earliest} to {latest}."
                        )
            except Exception as e:
        ...
        if collections and feature_count > 0:
            ...
            described_collections = [
                collection_descriptions.get(c, c) for c in collections
            ]

            if len(described_collections) == 1:
                response_parts.append(
                    f"The data comes from {described_collections[0]} satellite."
                )
            else:
                response_parts.append(
                    f"The data includes imagery from: {', '.join(described_collections)}."
                )
        ...
        if any(
            term in query_lower
            for term in ["elevation", "dem", "topography", "terrain"]
        ):
            response_parts.append(
                "This elevation data can be used for terrain analysis, slope calculations, watershed mapping, and 3D visualization."
            )
        elif any(
            term in query_lower
            for term in ["optical", "rgb", "satellite", "imagery", "urban", "city"]
        ):
            response_parts.append(
                "This optical imagery is perfect for visual analysis, urban planning, change detection, and land cover mapping. You can see buildings, roads, vegetation, and water features clearly."
            )
        elif any(term in query_lower for term in ["radar", "sar"]):
            response_parts.append(
                "This radar data can penetrate clouds and is excellent for all-weather monitoring, including surface changes and ground deformation."
            )
        elif any(term in query_lower for term in ["fire", "wildfire", "burn"]):
            response_parts.append(
                "This thermal and optical data is ideal for fire monitoring, burn area assessment, and recovery tracking."
            )
        else:
            response_parts.append(
                "This satellite data provides comprehensive coverage suitable for environmental monitoring and analysis."
            )

        response_parts.append(
            "All data is ready for analysis and has been loaded on the map for interactive exploration."
        )

        final_response = " ".join(response_parts)
        logger.info(
            f"Generated fallback response ({len(final_response)} chars): {final_response[:100]}..."
        )
...
def build_stac_query(stac_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if stac_params.get("datetime"):
        ...
        logger.info(
            f"[DATE] Adding datetime filter to STAC query: {stac_params['datetime']}"
        )
    ...
    if stac_params.get("sortby"):
        ...
        logger.info(
            "[CHART] Adding default sortby (datetime desc) to get most recent imagery"
        )
...
def clean_tilejson_urls(stac_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"[CLEAN] clean_tilejson_urls() called with stac_results type: {type(stac_results)}"
    )
    logger.info(
        f"[CLEAN] stac_results keys: {list(stac_results.keys()) if isinstance(stac_results, dict) else 'Not a dict'}"
    )
    ...
    try:
            ...
            if "assets" in cleaned_feature and cleaned_feature["assets"]:
                ...
                for asset_name, asset_data in cleaned_feature["assets"].items():
                    ...
                    if asset_name == "tilejson" and "href" in cleaned_asset:
                        ...
                        expression_collections = [
                            "alos-palsar-mosaic",
                            "sentinel-1-grd",
                            "sentinel-1-rtc",
                        ]
                        if (
                            collection_id in expression_collections
                ...
                if not has_tilejson and config:
                    ...
                    logger.warning(
                        f"[CLEAN] No tilejson and no PC config for {collection_id}/{item_id}"
                    )
        ...
        collection_name = (
            cleaned_features[0].get("collection") if cleaned_features else "N/A"
        )
        logger.info(
            f"[CLEAN] URL cleaning done — {len(cleaned_features)} features, collection={collection_name}"
        )
...
def _enhance_tilejson_url(url: str, collection_id: str) -> str:
    ...
    try:
        ...
        if config_dict.get("color_formula"):
            ...
            logger.info(
                f"[ART] Injecting color_formula: {config_dict['color_formula']}"
            )
...
async def health_check():
    ...
    try:
        ...
        logger.info(
            f"[BLDG] Health: {overall} | llm={checks['llm_client']['status']} pc={checks['planetary_computer']['status']} private_stac={checks['private_stac_api']['status']}"
        )
...
async def get_stac_collections():
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            async with session.get(collections_url) as resp:
                if resp.status == 200:
                    ...
                    return JSONResponse(
                        content={"configured": True, "collections": cols}
                    )
                return JSONResponse(
                    content={
                        "configured": True,
        ...
        return JSONResponse(
            content={"configured": True, "collections": [], "error": str(e)}
        )
...
async def unified_query_processor(body: QueryRequest):
    ...
    try:
        ...
        if semantic_translator:
            ...
            logger.warning(
                "[WARN] semantic_translator is None - using keyword-based fallback for collection detection."
            )
        ...
        log_pipeline_step(
            ...
            "INPUT",
            {
                ...
                "has_pin": bool(pin),
                "pin_coords": (
                    f"({pin.get('lat'):.4f}, {pin.get('lng'):.4f})" if pin else None
                ),
            },
        )
        ...
        logger.info(
            f"[TEXT] Query='{natural_query}' session={session_id} pin={pin_str}"
        )
        ...
        if is_quickstart_query(natural_query):
            ...
            logger.info(
                f"[FAST] QUICKSTART FAST PATH: '{natural_query}' -> {qs_location['collections']} @ {qs_location['location']}"
            )
            ...
            qs_stac_response = await execute_direct_stac_search(
                qs_stac_query, qs_stac_endpoint, original_query=natural_query
            )
            ...
            if qs_features:
                ...
                qs_date = (
                    qs_props.get("datetime")
                    or qs_props.get("start_datetime")
                    or qs_props.get("end_datetime")
                    or ""
                )
            ...
            qs_bboxes = []
            qs_collection_id = (
                qs_location["collections"][0] if qs_location["collections"] else ""
            )
            ...
            for feat in qs_features[:20]:
                ...
                if tj_asset and "href" in tj_asset:
                    ...
                    if not feat_bbox or len(feat_bbox) != 4:
                        continue
                    if any(
                        v is None or not isinstance(v, (int, float)) for v in feat_bbox
                    ):
                        continue

                    original_url = tj_asset["href"]
                    quality_params = build_tile_url_params(
                        qs_collection_id, natural_query
                    )
                    if quality_params and "?" in original_url:
                        base_url, existing_params = original_url.split("?", 1)
                        param_list = [
                            p
                            for p in existing_params.split("&")
                            if not any(
                                p.startswith(k)
                                for k in [
                                    "assets=",
                        ...
                        optimized_url = (
                            f"{base_url}?{'&'.join(param_list)}&{quality_params}"
                            if param_list
                            else f"{base_url}?{quality_params}"
                        )
                    elif quality_params:
            ...
            if any(
                qs_collection_id.startswith(oc) or qs_collection_id == oc
                for oc in optical_qs
            ):
                try:
            ...
            if router_agent and session_id and (qs_tile_urls or qs_features):
                    ...
                    stac_items_for_session.append(
                        {
                            ...
                            "properties": {
                                ...
                                "eo:cloud_cover": f.get("properties", {}).get(
                                    "eo:cloud_cover"
                                ),
                            },
                            "assets": {
                                k: {"href": v.get("href"), "type": v.get("type")}
                                for k, v in (f.get("assets") or {}).items()
                            },
                        }
                    )
                router_agent.update_session_context(
                    session_id,
                    {
                        ...
                        "query_count": router_agent.tools.session_contexts.get(
                            session_id, {}
                        ).get("query_count", 0)
                        + 1,
                    },
            ...
            logger.info(
                f"[FAST] QUICKSTART FAST PATH completed in {_qs_elapsed:.0f}ms (skipped RouterAgent + translate_query + response GPT)"
            )
        ...
        if router_agent and session_id:
            ...
            recently_navigated_early = session_bbox_early and not session_ctx.get(
                "has_rendered_map", True
            )

            if session_bbox_early and (
                not frontend_map_bounds or recently_navigated_early
            ):
                override_reason_early = (
                    "no frontend bounds"
                    if not frontend_map_bounds
                    else "post-navigate_to (frontend bounds stale)"
                )
                corrected_bounds = {
                    "west": session_bbox_early[0],
                    "south": session_bbox_early[1],
                    "east": session_bbox_early[2],
                    "north": session_bbox_early[3],
                ...
                logger.info(
                    f"[PIN] EARLY BBOX OVERRIDE ({override_reason_early}): Using session bbox for {session_ctx.get('last_location')}: center=({corrected_bounds['center_lat']:.4f}, {corrected_bounds['center_lng']:.4f})"
                )
        ...
        if router_action is None and router_agent and session_id:
            try:
                ...
                if global_translator:
                    ...
                    # ================================================================
                    existing_router_context = router_agent.tools.session_contexts.get(
                        session_id, {}
                    )
                    ...
                    if not context.get("last_bbox") and existing_router_context.get(
                        "last_bbox"
                    ):
                        context["last_bbox"] = existing_router_context.get("last_bbox")
                    if not context.get("last_location") and existing_router_context.get(
                        "last_location"
                    ):
                        context["last_location"] = existing_router_context.get(
                            "last_location"
                        )
                    if not context.get(
                        "last_collections"
                    ) and existing_router_context.get("last_collections"):
                        context["last_collections"] = existing_router_context.get(
                            "last_collections"
                        )
                ...
                logger.info(
                    f"[ROUTE] RouterAgent decision: {router_action.get('action_type', 'unknown')}"
                )
                log_pipeline_step(
                    pipeline_session_id,
                    "ROUTER_AGENT",
                    "OUTPUT",
                    {
                        ...
                        "needs_vision": router_action.get(
                ...
                if router_action.get("action_type") == "navigate_to":
                    ...
                    location = router_action.get("location") or router_action.get(
                        "original_query", ""
                    )
                    ...
                    logger.info(
                        f"[MAP] NAVIGATE_TO ACTION: Navigating to '{location_display}' at zoom {zoom_level}"
                    )
                    ...
                    try:
                        ...
                        if bbox and len(bbox) == 4:
                            ...
                            logger.info(
                                f"[OK] Location resolved: {location_display} -> ({center_lat:.4f}, {center_lng:.4f})"
                            )
                            ...
                            if router_agent and session_id:
                                ...
                                router_agent.update_session_context(
                                    session_id, navigate_context
                                )
                                logger.info(
                                    f"[PIN] Updated session context with navigate_to location: {location_display}, bbox: {bbox}"
                                )
                            ...
                            logger.warning(
                                f"[WARN] Could not resolve location '{location}' via stored locations"
                            )
                            return {
                                "success": True,
                                "action": "contextual",
                ...
                if router_action.get("action_type") == "extreme_weather":
                    ...
                    logger.info(
                        f"[STORM] EXTREME_WEATHER ACTION: Climate query for '{location}'"
                    )
                    ...
                    try:
                        ...
                        bbox = (
                            await resolver.resolve_location_to_bbox(location)
                            if location
                            else None
                        )
                        ...
                        if not bbox and router_agent and session_id:
                            ctx = router_agent.tools.session_contexts.get(
                                session_id, {}
                            )
                            bbox = ctx.get("last_bbox")
                            location = location or ctx.get(
                        ...
                        if bbox and len(bbox) == 4:
                            ...
                            logger.warning(
                                "[WARN] No coordinates for climate query, falling through"
                            )
                        ...
                        if center_lat is not None and center_lng is not None:
                            ...
                            user_query = router_action.get(
                                "original_query", "Provide a climate overview."
                            )
                            ...
                            analysis = result.get(
                                "response", "Climate analysis complete."
                            )
                            location_display = (
                                location.title() if location else "this location"
                            )

                            logger.info(
                                f"[STORM] Climate analysis complete for {location_display}"
                            )
                        ...
                        logger.error(
                            f"[STORM] Climate analysis timed out for {location}"
                        )
                        return {
                            "success": False,
                        ...
                        logger.error(
                            f"[STORM] Climate analysis failed: {climate_error}"
                        )
                        # Fall through to STAC search as fallback

            except Exception as router_error:
                logger.warning(
                    f"[WARN] RouterAgent failed: {router_error}, falling back to legacy classification"
                )
                router_action = None
        ...
        if router_action:
            ...
            if action_type == "stac_search":
                logger.info(
                    "Converting RouterAgent 'stac_search' to classification format"
                )
                classification = {
                    "intent_type": "stac",
                    "confidence": 0.95,
                ...
                logger.info(
                    "Converting RouterAgent 'vision_analysis' to classification format"
                )
                classification = {
                    "intent_type": "vision",
                    "confidence": 0.95,
                ...
                logger.info(
                    "Converting RouterAgent 'contextual' to classification format"
                )
                classification = {
                ...
                logger.info("Converting RouterAgent 'hybrid' to classification format")
                classification = {
                    "intent_type": "hybrid",
                    "confidence": 0.95,
                    "needs_satellite_data": True,
            ...
            if action_type == "navigate_to":
                logger.warning(
                    "[WARN] navigate_to reached classification bridge (should have returned early). Treating as contextual."
                )
                classification = {
                    "intent_type": "contextual",
                    "confidence": 0.7,
        ...
        if SEMANTIC_KERNEL_AVAILABLE and global_translator:
            try:
                ...
                if quickstart_classification and quickstart_location:
                    logger.info(
                        f"[LAUNCH] QUICK START QUERY DETECTED: '{natural_query}'"
                    )
                    logger.info(
                        f"   Pre-computed: collection={quickstart_location['collections']}, location={quickstart_location['location']}"
                    )
                ...
                if not classification:
                    ...
                    logger.warning(
                        "RouterAgent did not provide classification, defaulting to STAC search"
                    )
                    classification = {
                        "intent_type": "stac",
                        "confidence": 0.5,
                    ...
                    logger.info(
                        "Skipping classification - Quick start query with pre-computed result"
                    )
                else:
                    logger.info("Using RouterAgent classification")

                intent_type = classification.get(
                    "intent_type", "stac"
                )  # Default to STAC query
                confidence = classification.get("confidence", 0)
                modules_to_execute = classification.get("modules", [])

                logger.info("CLASSIFICATION RESULT (from RouterAgent):")
                logger.info(f"   Query: '{natural_query}'")
                logger.info(f"   Intent: {intent_type}")
                logger.info(f"   Confidence: {confidence:.2f}")
                logger.info(
                    f"   Needs satellite data: {classification.get('needs_satellite_data', False)}"
                )
                logger.info(
                    f"   Needs contextual info: {classification.get('needs_contextual_info', False)}"
                )
                logger.info(
                    f"   Needs vision analysis: {classification.get('needs_vision_analysis', False)}"
                )
                ...
                log_pipeline_step(
                    ...
                    "OUTPUT",
                    {
                        ...
                        "needs_satellite_data": classification.get(
                            "needs_satellite_data", False
                        ),
                        "needs_vision": classification.get(
                            "needs_vision_analysis", False
                        ),
                ...
                conversation_history = req_body.get(
                    "conversation_history", []
                ) or req_body.get("messages", [])
                ...
                router_says_stac = (
                    classification and classification.get("intent_type") == "stac"
                )

                if router_says_stac:
                    needs_vision = False
                    logger.info(
                        "[CHART] RouterAgent classified as STAC search — skipping vision keyword detection"
                    )
                else:
                    ...
                    needs_vision = chat_vision_detector.should_use_vision(
                        natural_query, conversation_history
                    )

                    if needs_vision:
                        logger.info(
                            "[SEARCH] Vision query detected by KEYWORD/CONTEXT matching (priority check)"
                        )
                    else:
                        ...
                        if needs_vision:
                            logger.info(
                                "[SEARCH] Vision query detected by GPT CLASSIFICATION (fallback check)"
                            )
                ...
                if needs_vision:
                    logger.info(
                        "[IMG] VISION QUERY DETECTED: User is asking about visible imagery"
                    )

                    # Check if frontend provided map context
                    map_bounds = req_body.get(
                        "map_bounds"
                    )  # {north, south, east, west, center_lat, center_lng}
                    imagery_url = req_body.get(
                        "imagery_url"
                    )  # Current visible imagery URL
                    imagery_base64 = req_body.get(
                        "imagery_base64"
                    )  # Base64 screenshot from frontend
                    current_collection = req_body.get(
                        "current_collection"
                    )  # Collection ID (string)
                    tile_urls = req_body.get(
                        "tile_urls", []
                    )  # TiTiler URLs from prior STAC response
                    has_satellite_data = req_body.get(
                        "has_satellite_data", False
                    )  # Flag if STAC imagery loaded
                    conversation_history = req_body.get(
                        "conversation_history", []
                    ) or req_body.get("messages", [])
                    ...
                    if router_agent and session_id:
                        session_context = router_agent.tools.session_contexts.get(
                            session_id, {}
                        )
                        logger.info(
                            f"[LIST] Session context for Vision Agent: {list(session_context.keys())}"
                        )

                        # Merge session collections with current collection
                        session_collections = session_context.get(
                            "last_collections", []
                        )
                        ...
                        stac_items_from_session = session_context.get(
                            "last_stac_items", []
                        )
                        if stac_items_from_session:
                            logger.info(
                                f"[PKG] Retrieved {len(stac_items_from_session)} STAC items from session for vision agent"
                            )
                        ...
                        recently_navigated = session_bbox and not session_context.get(
                            "has_rendered_map", True
                        )
                        ...
                        if session_bbox and (not map_bounds or recently_navigated):
                            ...
                            logger.info(
                                f"[PIN] Using session bbox for Vision Agent ({override_reason}): {session_context.get('last_location')}"
                            )

                    # Convert collection to list for Vision Agent (typically one collection per query)
                    if (
                        current_collection
                        and current_collection not in collections_list
                    ):
                        collections_list = [current_collection] + collections_list

                    # Log vision context availability
                    logger.info(
                        f"[SNAP] Map screenshot available: {bool(imagery_base64)}"
                    )
                    ...
                    if is_hybrid_vision:
                        logger.info(
                            "[SHUFFLE] HYBRID VISION + DATA QUERY: Will run vision + STAC in PARALLEL for optimal performance"
                        )
                    ...
                    if map_bounds or imagery_url or imagery_base64:
                        try:
                            ...
                            if is_hybrid_vision:
                                ...
                                logger.info(
                                    "[FAST] Starting Vision Agent task (parallel execution)..."
                                )
                                ...
                                logger.info(
                                    "[SYNC] Vision Agent task created, continuing to STAC data retrieval (parallel)..."
                                )
                            else:
                                # Pure vision query - invoke Vision Agent and return immediately
                                logger.info(
                                    "[BOT] Invoking Vision Agent for pure vision query"
                                )
                                logger.info(f"   Collections: {collections_list}")
                                logger.info(f"   Tile URLs: {len(tile_urls)} tiles")
                                logger.info(
                                    f"   STAC items: {len(stac_items_from_session)} items"
                                )
                                logger.info(
                                    f"   Has screenshot: {bool(imagery_base64)}"
                                )
                                ...
                                # ================================================================
                                logger.info(
                                    f"[SEARCH] Vision Agent returned: {type(vision_result)}"
                                )
                                if vision_result:
                                    logger.info(
                                        f"   Keys: {list(vision_result.keys())}"
                                    )
                                    logger.info(
                                        f"   Response: '{str(vision_result.get('response', ''))[:100]}...'"
                                    )
                                    logger.info(
                                        f"   Analysis: '{str(vision_result.get('analysis', ''))[:100]}...'"
                                    )
                                ...
                                if response_text:
                                    ...
                                    logger.info(
                                        f"   Cached: {vision_result.get('cached', False)}"
                                    )
                                    ...
                                    logger.info(
                                        f"[TOOL] Vision Agent tools used: {tools_used}"
                                    )
                                    ...
                                    log_pipeline_step(
                                        ...
                                        "OUTPUT",
                                        {
                                            ...
                                            "has_screenshot": vision_result.get(
                                                "context", {}
                                            ).get("has_screenshot", False),
                                            "collections": vision_result.get(
                                                "context", {}
                                            ).get("collections", []),
                                    ...
                                    # ================================================================
                                    logger.warning(
                                        f"[WARN] Vision Agent returned empty response. Result: {vision_result}"
                                    )
                                    log_pipeline_step(
                                        pipeline_session_id,
                                        "VISION_AGENT",
                            ...
                            logger.warning(
                                f"[WARN] Chat vision analysis failed: {e}, returning error..."
                            )
                        ...
                        # ================================================================
                        logger.info(
                            "ℹ️ No map screenshot, but Vision Agent can use LLM tool"
                        )
                        try:
                            vision_result = await vision_agent.analyze(
                                user_query=natural_query,
                            ...
                            logger.info(
                                f"[SEARCH] Vision Agent (LLM mode) returned: {type(vision_result)}"
                            )
                            if vision_result:
                            ...
                            if response_text:
                                ...
                                tool_calls = vision_result.get("tool_calls", [])
                                logger.info(
                                    f"[TOOL] Vision Agent (LLM mode) tools used: {tools_used}"
                                )
                                ...
                                # ================================================================
                                logger.warning(
                                    f"[WARN] Vision Agent (LLM mode) returned empty. Result: {vision_result}"
                                )
                            ...
                            logger.warning(
                                f"[WARN] Vision Agent LLM fallback failed: {e}"
                            )
                            logger.error(traceback.format_exc())
                ...
                if (
                    ...
                    logger.info(
                        "[MSG] CONTEXTUAL REQUEST: Skipping STAC search, generating educational response..."
                    )
                    ...
                    if router_agent and session_id:
                        ...
                        if session_location or session_bbox or current_map_bounds:
                            ...
                            if location_hint:
                                ...
                                logger.info(
                                    f"[PIN] Enriched contextual query with location: {location_hint}"
                                )
                    ...
                    try:
                        contextual_response = await asyncio.wait_for(
                            translator.generate_contextual_earth_science_response(
                                ...
                                classification,
                                {
                                    "success": True,
                                    "results": {"features": []},
                                },  # Empty STAC response for contextual-only
                            ),
                            timeout=30.0,
                        )
                        ...
                        if contextual_response and contextual_response.get("message"):
                            logger.info(
                                "[OK] Generated contextual response successfully"
                            )
                            return {
                                "success": True,
                        ...
                        logger.warning(
                            f"[WARN] Contextual response generation failed: {e}, proceeding with STAC search..."
                        )
        ...
        if classification and intent_type:
            if intent_type in ["vision", "contextual"]:
                ...
                logger.info(
                    f"[SKIP] Skipping collection mapping for {intent_type} query (no STAC search needed)"
                )

        if not skip_collection_mapping:
            if not SEMANTIC_KERNEL_AVAILABLE:
                logger.warning(
                    "[WARN] Semantic Kernel not available, using fallback processing"
                )
                ...
                if collections:
                    ...
                    logger.info(
                        f"[TOOL] Built fallback STAC query: {json.dumps(stac_query, indent=2)}"
                    )

            elif SEMANTIC_KERNEL_AVAILABLE and translator:
                try:
                    logger.info(
                        f"[BOT] Starting multi-agent pipeline — query='{natural_query}' pin={pin}"
                    )
                    ...
                    if router_agent and session_id:
                        try:
                            ...
                            if session_bbox:
                                logger.info(
                                    f"[PIN] Session fallback available: {session_location} bbox={session_bbox}"
                                )
                        except AttributeError:
                            logger.debug(
                                "[PIN] No session context available (router_agent.tools.session_contexts not found)"
                            )

                    # If router explicitly said use_current_location, prioritize it
                    if router_action and router_action.get("use_current_location"):
                        logger.info("[PIN] Router set use_current_location=True")

                    logger.info(
                        "[SYNC] Translating natural language to STAC parameters..."
                    )
                    ...
                    if quickstart_data:
                        logger.info(
                            "[LAUNCH] Using pre-computed STAC params for quick start query"
                        )
                        ...
                        if temporal:
                            ...
                            if len(temporal) == 4:  # Year only: "2017"
                                ...
                                datetime_range = (
                                    f"{temporal}-01/{temporal}-{last_day:02d}"
                                )
                            ...
                            logger.info(
                                f"[DATE] Converted temporal '{temporal}' to datetime range: {datetime_range}"
                            )
                        ...
                        stac_params = await translator.translate_query(
                            natural_query, pin_location=pin, session_bbox=session_bbox
                        )
                    ...
                    if stac_params and stac_params.get("error") == "LOCATION_REQUIRED":
                        logger.warning(
                            "[WARN] Location required but not found in query"
                        )
                        return {
                            "success": False,
                            "error": "LOCATION_REQUIRED",
                            "response": stac_params.get(
                    ...
                    if stac_params and stac_params.get("collections"):
                        logger.info(
                            f"[OK] Translation successful: {len(stac_params.get('collections', []))} collections selected"
                        )
                        ...
                        stac_query = build_stac_query(stac_params)
                        logger.info(
                            f"[TOOL] Built STAC query from params: {json.dumps(stac_query, indent=2)}"
                        )
                        ...
                        if not stac_query.get("datetime") and natural_query:
                            ...
                            if month_year_match:
                                m_name, y_str = month_year_match.group(
                                    1
                            ...
                            if not extracted_datetime:
                                ...
                                if month_year_match2:
                                    m_name, y_str = month_year_match2.group(
                                        1
                                    ), month_year_match2.group(2)
                                    m_num = month_names[m_name]
                                    y_num = int(y_str)
                                    last_day = cal_mod.monthrange(y_num, m_num)[1]
                                    extracted_datetime = f"{y_str}-{m_num:02d}-01/{y_str}-{m_num:02d}-{last_day:02d}"
                            ...
                            if not extracted_datetime:
                                year_match = re.search(
                                    r"(?:from|in|for)\s+((?:19|20)\d{2})\b", query_lower
                                )
                                if year_match:
                            ...
                            if extracted_datetime:
                                ...
                                logger.info(
                                    f"[DATE] REGEX FALLBACK: Extracted datetime '{extracted_datetime}' from query (GPT agent missed it)"
                                )

                    else:
                        logger.warning(
                            f"[WARN] Translation did not produce valid STAC parameters. Got: {stac_params}"
                        )
                    ...
                    logger.error(
                        f"[FAIL] Semantic translation failed: {e}", exc_info=True
                    )
        ...
        if stac_query:
            try:
                ...
                stac_endpoint = translator.determine_stac_source(
                    natural_query, stac_params
                )
                ...
                stac_response = await execute_direct_stac_search(
                    stac_query, stac_endpoint, original_query=natural_query
                )
                ...
                if stac_endpoint != "planetary_computer":
                    local_features = stac_response.get("results", {}).get(
                        "features", []
                    )
                    if not local_features:
                        logger.info(
                            "[SIGNAL] Local STAC returned 0 features — retrying against Planetary Computer"
                        )
                ...
                if stac_response.get("success"):
                    ...
                    logger.info(
                        f"[OK] STAC search completed: {len(raw_features)} raw features found"
                    )
                    ...
                    log_pipeline_step(
                        ...
                        "OUTPUT",
                        {
                            ...
                            "first_feature_id": (
                                raw_features[0].get("id") if raw_features else None
                            ),
                            "first_feature_bbox": (
                                raw_features[0].get("bbox") if raw_features else None
                            ),
                        },
                    )
                    ...
                    if is_modis and requested_bbox and raw_features:
                        logger.info(
                            f"[MAP]  MODIS BBOX FILTER: Checking {len(raw_features)} tiles against requested bbox"
                        )
                        logger.info(
                            f"   Requested bbox: [{requested_bbox[0]:.2f}, {requested_bbox[1]:.2f}, {requested_bbox[2]:.2f}, {requested_bbox[3]:.2f}]"
                        )
                        ...
                        for feature in raw_features:
                            ...
                            if (
                                req_west <= tile_center_lon <= req_east
                                and req_south <= tile_center_lat <= req_north
                            ):
                                filtered_features.append(feature)
                            else:
                                logger.debug(
                                    f"   [FAIL] Filtered out tile with center ({tile_center_lon:.2f}, {tile_center_lat:.2f}) outside bbox"
                                )

                        logger.info(
                            f"[OK] MODIS BBOX FILTER: Kept {len(filtered_features)}/{len(raw_features)} tiles within requested bbox"
                        )
                    ...
                    if (
                        ...
                        logger.info(
                            f"[SKIP] Skipping spatial pre-filter: Agent 3 will ensure coverage from {len(raw_features)} tiles"
                        )
                        ...
                        if stac_params and stac_params.get("cloud_cover") is not None:
                            ...
                            for prop_name in ["eo:cloud_cover", "cloud_cover"]:
                                if prop_name in query_filter:
                                    ...
                                    logger.info(
                                        f"[SEARCH] DEBUG: {prop_name} NOT in query_filter"
                                    )
                        ...
                        if cloud_cover_limit is None and natural_query:
                            ...
                            if any(kw in query_lower for kw in low_cloud_keywords):
                                ...
                                logger.info(
                                    f"[TOOL] FALLBACK: Detected low cloud keywords in query, applying {cloud_cover_limit}% filter"
                                )

                        logger.info(
                            f"[SEARCH] DEBUG: Final cloud_cover_limit = {cloud_cover_limit}"
                        )
                            ...
                            for threshold in relaxation_thresholds:
                                ...
                                if filtered_features or threshold == 100:
                                    ...
                                    if threshold > original_cloud_limit:
                                        ...
                                        logger.info(
                                            f"[CLOUD] Cloud filter relaxed: {original_cloud_limit}% -> {threshold}% (found {len(filtered_features)} tiles)"
                                        )
                                        search_diagnostics["cloud_relaxation"] = (
                                            f"{original_cloud_limit}% -> {threshold}%"
                                        )
                                    else:
                                        logger.info(
                                            f"[CLOUD] After cloud cover filtering (≤{threshold}%): {len(filtered_features)} features kept"
                                        )
                                    break
                        ...
                        logger.info(
                            "[LIST] Using STAC API ordering (results already sorted by datetime desc)"
                        )

                        # Use all spatially filtered features (already ordered by recency from STAC API)
                        features = spatially_filtered_features
                        search_diagnostics["final_count"] = len(features)
                        search_diagnostics["tile_selection_method"] = (
                            "stac_api_ordering"
                        )
                        logger.info(
                            f"[OK] Using {len(features)} tiles from STAC API (pre-sorted by datetime desc)"
                        )
                        ...
                        search_diagnostics["final_count"] = len(features)
                        logger.info(
                            "[WARN] Skipping spatial filtering and tile ranking (no translator or bbox)"
                        )
                    ...
                    if search_diagnostics["final_count"] == 0:
                        ...
                        logger.info(
                            "[SYNC] No results found - attempting to find alternatives with relaxed filters..."
                        )
                        logger.info(
                            f"[SYNC] STAC query datetime: {stac_query.get('datetime')}, collections: {stac_query.get('collections')}, bbox: {stac_query.get('bbox')}"
                        )
                        try:
                            ...
                            logger.info(
                                f"[SYNC] Alternative result: success={alternative_result.get('success')}, features={len(alternative_result.get('features', []))}"
                            )
                        except Exception as alt_ex:
                            logger.error(
                                f"[SYNC] EXCEPTION in try_alternative_queries: {type(alt_ex).__name__}: {alt_ex}"
                            )
                            logger.error(f"[SYNC] Traceback: {traceback.format_exc()}")
                            alternative_result = {"success": False, "features": []}

                        if alternative_result.get("success") and alternative_result.get(
                            "features"
                        ):
                            ...
                            search_diagnostics["relaxation_applied"] = (
                                alternative_result.get("relaxation_applied")
                            )
                            search_diagnostics["alternative_explanation"] = (
                                alternative_result.get("explanation")
                            )
                            search_diagnostics["original_filters"] = (
                                alternative_result.get("original_filters")
                            )
                            search_diagnostics["alternative_filters"] = (
                                alternative_result.get("alternative_filters")
                            )
                            ...
                            logger.info(
                                f"[OK] Found {len(features)} alternative results: {alternative_result.get('explanation')}"
                            )
                        else:
                            logger.info(
                                "[FAIL] No alternatives found - will generate empty result response"
                            )
                else:
                    logger.error(
                        f"[FAIL] STAC search failed: {stac_response.get('error', 'Unknown error')}"
                    )
        ...
        if (
            stac_params
            and stac_params.get("geoint_processing")
            and GEOINT_AVAILABLE
            and geoint_executor
            and features
        ):
            try:
                logger.info(
                    "[MICRO] Query requires GEOINT analysis - executing analytical processing..."
                )
                logger.info(
                    f"[CHART] GEOINT intent detected: {stac_params.get('analysis_intent', {})}"
                )
                ...
                if geoint_results and geoint_results.get("success"):
                    ...
                    logger.info(
                        f"[UP] Analysis results preview: {list(geoint_results.get('results', {}).keys())}"
                    )

                    # Log key metrics for debugging
                    if analysis_type == "terrain_analysis":
                        elevation_stats = geoint_results.get("results", {}).get(
                            "elevation_statistics", {}
                        )
                        if elevation_stats:
                            logger.info(
                                f"[MTN] Elevation: min={elevation_stats.get('min_elevation'):.1f}m, "
                            ...
                            if peak_loc:
                                logger.info(
                                    f"[PIN] Peak location: ({peak_loc.get('latitude'):.4f}, "
                                    f"{peak_loc.get('longitude'):.4f}) @ {peak_loc.get('elevation'):.1f}m"
                                )

                    elif analysis_type == "mobility_analysis":
                        mobility_zones = geoint_results.get("results", {}).get(
                            "mobility_zones", {}
                        )
                        if mobility_zones:
                            go_pct = mobility_zones.get("go_zones", {}).get(
                                "percentage", 0
                            )
                            logger.info(
                                f"[CAR] Mobility: {go_pct:.1f}% accessible terrain"
                            )
                else:
                    logger.warning(
                        "[WARN] GEOINT analysis completed but returned no results or failed"
                    )

            except Exception as e:
                logger.error(
                    f"[FAIL] GEOINT analysis execution failed: {str(e)}", exc_info=True
                )
                geoint_results = None
        elif stac_params and stac_params.get("geoint_processing"):
            logger.warning(
                "[WARN] GEOINT processing requested but prerequisites not met:"
            )
        ...
        logger.info("[SEARCH] RESPONSE GENERATION DEBUG:")
        logger.info(f"   - features count: {len(features) if features else 0}")
        logger.info(
            f"   - stac_response features: {len(stac_response.get('results', {}).get('features', []))}"
        )
        ...
        if not features and stac_response_features:
            logger.warning(
                f"[WARN] MISMATCH DETECTED: features is empty but stac_response has {len(stac_response_features)} features!"
            )
            logger.warning(
                "   Synchronizing features from stac_response to prevent 'no matches' bug"
            )
        ...
        if SEMANTIC_KERNEL_AVAILABLE and translator and features:
            try:
                ...
                if "vision_task" in locals() and vision_task is not None:
                    try:
                        logger.info(
                            "[WAIT] Awaiting parallel vision analysis to complete..."
                        )
                        ...
                        if vision_result and vision_result.get("analysis"):
                            ...
                            logger.info(
                                "[OK] Parallel vision analysis completed and ready for combination"
                            )
                        else:
                            logger.warning(
                                "[WARN] Vision task completed but no analysis returned"
                            )
                    except Exception as e:
                ...
                if search_diagnostics.get("alternative_used"):
                    ...
                    contextual_response = (
                        await translator.generate_alternative_result_response(
                            natural_query,
                            classification or {},
                    ...
                    response_message = contextual_response.get(
                        "message", f"Found {len(features)} alternative results."
                    )
                else:
                    # Standard success response
                    contextual_response = (
                        await translator.generate_contextual_earth_science_response(
                            natural_query,
                            classification or {},
                            {"success": True, "results": {"features": features}},
                            geoint_results,
                    ...
                    response_message = contextual_response.get(
                        "message",
                        f"Found {len(features)} satellite data results for your query.",
                    )

                    # If we have vision analysis from hybrid query, prepend it
                    if early_contextual_response:
                        logger.info(
                            "[SHUFFLE] Combining vision analysis with STAC data response"
                        )
...
{response_message}"""
                ...
                response_message = generate_fallback_response(
                    natural_query,
                    features,
            ...
            try:
                logger.info(
                    f"[BOT] Generating GPT-powered empty result response (failure stage: {search_diagnostics.get('failure_stage')})"
                )
                ...
                logger.error(
                    f"[FAIL] GPT empty result generation failed, using rule-based fallback: {e}"
                )
                # Fall back to rule-based response if GPT fails
                response_message = generate_contextual_empty_response(
                    natural_query, stac_query.get("collections", []), search_diagnostics
                )
        elif not features and stac_query:
            # Fallback: Use rule-based response if Semantic Kernel not available
            logger.info(
                f"ℹ️ No features found - using rule-based diagnostic response (failure stage: {search_diagnostics.get('failure_stage')})"
            )
            response_message = generate_contextual_empty_response(
                natural_query, stac_query.get("collections", []), search_diagnostics
            )
        ...
        mosaic_result = (
            None  # [GLOBE] NEW: Mosaic tilejson result for seamless coverage
        )
        ...
        if features:
            ...
            logger.info(
                f"[SEARCH] COLLECTION DEBUG: Requested={collections}, Features contain={feature_collections}"
            )
            if set(feature_collections) != set(collections):
                logger.warning(
                    "[WARN] MISMATCH: STAC returned different collections than requested!"
                )
        ...
        if needs_mosaic and stac_query and stac_query.get("bbox"):
            logger.info(
                f"[GLOBE] MOSAIC: Enabling mosaic for optical collection {collection_id}"
            )
            try:
                ...
                mosaic_result = await get_mosaic_tilejson_url(
                    ...
                    query_filters=(
                        ...
                        if "cloud" not in natural_query.lower()
                        else None
                    ),
                    query_context=natural_query,
                )
                ...
                if mosaic_result:
                    logger.info(
                        f"[OK] MOSAIC: Successfully registered mosaic for {collection_id}"
                    )
                else:
                    logger.warning(
                        f"[WARN] MOSAIC: Registration failed for {collection_id}, will use item tiles"
                    )
            except Exception as e:
            ...
            if not needs_mosaic:
                logger.info(
                    f"[MAP] MOSAIC: Skipping mosaic for {collection_id} (not an optical collection)"
                )
        ...
        if features and len(features) > 0:
            ...
            logger.info(
                f"[MAP] Generating tile URLs for {len(top_features)} features (of {len(features)} total) for {collection_id}"
            )
            ...
            for feature in top_features:
                ...
                if tilejson_asset and "href" in tilejson_asset:
                    ...
                    if not feature_bbox or len(feature_bbox) != 4:
                        logger.warning(
                            f"[WARN] Skipping feature {feature.get('id')} - invalid bbox: {feature_bbox}"
                        )
                        continue

                    if any(
                        v is None or not isinstance(v, (int, float))
                        for v in feature_bbox
                    ):
                        logger.warning(
                            f"[WARN] Skipping feature {feature.get('id')} - bbox contains null values: {feature_bbox}"
                        )
                        continue

                    collection_id = feature.get(
                    ...
                    if (
                        ...
                        logger.info(
                            f"[SHIELD] Keeping original STAC tilejson for expression-based collection {collection_id}"
                        )
                    ...
                    if quality_params:
                        ...
                        logger.info(
                            f"[ART] Optimized tile URL for {feature.get('id')}: {quality_params}"
                        )
                    else:
                        optimized_url = original_url
                        logger.info(
                            f"[WARN] No quality params generated for {feature.get('id')}"
                        )
            ...
            if all_tile_urls:
                logger.info(
                    f"[MAP] Generated {len(all_tile_urls)} tile URLs for bbox coverage (skipped {skipped_count} with invalid bbox)"
                )
                ...
                log_pipeline_step(
                    ...
                    "OUTPUT",
                    {
                        ...
                        "tile_ids": [
                            t.get("item_id") for t in all_tile_urls[:5]
                        ],  # First 5 for brevity
                ...
                if all_bboxes:
                    ...
                    logger.info(
                        f"   Coverage: {union_bbox[2] - union_bbox[0]:.2f}° × {union_bbox[3] - union_bbox[1]:.2f}°"
                    )
                    ...
                    if stac_params:
                        ...
                        logger.info(
                            f"[OK] Updated stac_params bbox to union: {union_bbox}"
                        )
        ...
        if (
            not all_tile_urls
            and not mosaic_result
            and features
            and stac_endpoint != "planetary_computer"
        ):
            logger.info(
                "[SIGNAL] Local STAC features have no tilejson assets — retrying against Planetary Computer"
            )
            pc_collections = detect_collections(natural_query)
            pc_stac_query = {**stac_query, "collections": pc_collections}
            pc_stac_response = await execute_direct_stac_search(
                pc_stac_query, "planetary_computer", original_query=natural_query
            )
            if pc_stac_response.get("success"):
                pc_features = pc_stac_response.get("results", {}).get("features", [])
                if pc_features:
                    logger.info(
                        f"[OK] PC fallback returned {len(pc_features)} renderable features"
                    )
                    ...
                    all_tile_urls = []
                    all_bboxes = []
                    collection_id = pc_features[0].get(
                    ...
                    for feature in pc_features[:20]:
                        ...
                        if tilejson_asset and "href" in tilejson_asset:
                            ...
                            if any(
                                ...
                                continue
                            feat_collection_id = feature.get(
                                "collection", collection_id
                            )
                            original_url = tilejson_asset["href"]
                            quality_params = build_tile_url_params(
                                feat_collection_id, natural_query
                            )
                            if quality_params and "?" in original_url:
                                base_url, existing_params = original_url.split("?", 1)
                                param_list = [
                    ...
                    if not mosaic_result and stac_query and stac_query.get("bbox"):
                        pc_collection_id = (
                            pc_features[0].get("collection", "") if pc_features else ""
                        )
                        optical_collections_needing_mosaic = [
                            "sentinel-2-l2a",
                            "landsat-c2-l2",
                        ...
                        if any(
                            pc_collection_id.startswith(oc) or pc_collection_id == oc
                            for oc in optical_collections_needing_mosaic
                        ):
                            try:
                                from hybrid_rendering_system import (
                                    get_mosaic_tilejson_url,
                                )
        ...
        complete_response = {
            ...
            "data": {
                ...
                "search_metadata": {
                    ...
                    "temporal_range": (
                        stac_query.get("datetime") if stac_query else None
                    ),
                    "search_timestamp": datetime.utcnow().isoformat(),
                },
                "query_classification": (
                    {
                        ...
                        "needs_satellite_data": (
                            classification.get("needs_satellite_data")
                            if classification
                            else True
                        ),
                        "needs_contextual_info": (
                            classification.get("needs_contextual_info")
                            if classification
                            else False
                        ),
                        "confidence": (
                            classification.get("confidence") if classification else 0
                        ),
                    }
                    if classification
                    else None
            ...
            "translation_metadata": {
                ...
                "translation_timestamp": datetime.utcnow().isoformat(),
                "all_tile_urls": (
                    all_tile_urls if all_tile_urls else None
                ),  # [+] Individual tile URLs for multi-tile rendering
        ...
        if router_agent and session_id and (all_tile_urls or mosaic_result or features):
            ...
            for f in (features or [])[
                :10
            ]:  # Limit to 10 items to keep context manageable
            ...
            logger.info(
                f"[MAP] Updated session context: has_rendered_map=True, collections={stac_render_context['last_collections']}, stac_items={len(stac_items_for_vision)}"
            )
...
async def sign_mosaic_url(body: SignMosaicUrlRequest):
    ...
    try:
        ...
        return JSONResponse(
            content={"signed_url": signed_url, "authenticated": authenticated}
        )
...
async def stac_search(body: StacSearchRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="STAC search failed")
...
async def veda_search(body: VedaSearchRequest):
    ...
    try:
        ...
        stac_response = await execute_direct_stac_search(
            body.model_dump(exclude_none=True), stac_endpoint="veda"
        )
        ...
        raise HTTPException(status_code=500, detail="VEDA search failed")
...
async def structured_search(body: StructuredSearchRequest):
    ...
    try:
        ...
        logger.info("=" * 100)
        logger.info(
            "[TOOL][TOOL][TOOL] POST /api/structured-search ENDPOINT HIT [TOOL][TOOL][TOOL]"
        )
        logger.info("=" * 100)
        ...
        logger.info("[PKG] Structured search params:")
        ...
        if (
            not collection or not location
        ):  # belt-and-suspenders (Pydantic already enforces these)
            raise HTTPException(
                status_code=400,
        ...
        logger.info(
            "[BOT] Converting structured parameters to natural language for agent pipeline"
        )
        ...
        stac_params = await global_translator.translate_query(
            natural_query, pin_location=None
        )

        # Check if translation was successful
        if not stac_params or not stac_params.get("collections"):
            logger.error(
                "[FAIL] Agent pipeline failed to produce valid STAC parameters"
            )
            raise HTTPException(
                status_code=500,
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail="Structured search failed")
...
async def process_comparison_query(body: ComparisonQueryRequest):
    ...
    try:
        ...
        if not semantic_translator:
            raise HTTPException(
                status_code=500, detail="Semantic translator not initialized"
            )
        ...
        stac_query = await semantic_translator.build_stac_query_agent(
            user_query, collections
        )
        bbox = stac_query.get("bbox")
        ...
        lat = (bbox[1] + bbox[3]) / 2
        logger.info(
            f"[CHART] [COMPARISON] Step 2 location={location_name} ({lat:.4f},{lng:.4f})"
        )

        # Step 3: Dual-Date Extraction
        datetime_result = await semantic_translator.datetime_translation_agent(
            query=user_query, collections=collections, mode="comparison"
        )
        ...
        if "before" not in datetime_result or "after" not in datetime_result:
            logger.error(
                f"[CHART] [COMPARISON] Missing before/after in datetime result: {datetime_result}"
            )
            raise HTTPException(
                status_code=400,
        ...
        logger.info(
            f"[CHART] [COMPARISON] Step 3 before={before_date} after={after_date}"
        )
        ...
        logger.info(
            f"[CHART] [COMPARISON] Step 4 primary={primary_collection} aspect={aspect}"
        )
        ...
        logger.info(
            f"[CHART] [COMPARISON] [OK] Done — {location_name}, {primary_collection}, {before_date}->{after_date}"
        )
        return result
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process comparison query"
        )
...
async def geoint_mobility_analysis(body: MobilityRequest):
    ...
    try:
        ...
        logger.info("Mobility agent completed")
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail="GEOINT mobility analysis failed")
...
async def geoint_terrain_analysis(body: TerrainRequest):
    ...
    try:
        ...
        logger.info(
            f"[MTN] [{request_id}] Screenshot size: {screenshot_size} chars ({screenshot_size / 1024:.1f} KB)"
        )
        ...
        if screenshot:
            ...
            logger.info(
                f"[MTN] [{request_id}] Screenshot starts with: {screenshot[:50]}"
            )

            # Check if it has data URL prefix (should be removed by frontend)
            if screenshot.startswith("data:image"):
                logger.warning(
                    f"[MTN] [{request_id}] [WARN] Screenshot has data URL prefix (will be handled)"
                )
            else:
                logger.info(
                    f"[MTN] [{request_id}] Screenshot is pure base64 (correct format)"
                )

            # Validate minimum size (empty canvas produces very small PNGs)
            if screenshot_size < 5000:
                logger.warning(
                    f"[MTN] [{request_id}] [WARN] Screenshot is very small ({screenshot_size} chars) - may be blank canvas!"
                )
            else:
                logger.info(
                    f"[MTN] [{request_id}] Screenshot size looks good (>{screenshot_size / 1024:.1f}KB)"
                )
        else:
            logger.warning(
                f"[MTN] [{request_id}] [WARN] NO SCREENSHOT PROVIDED - will use STAC fallback"
            )
        # ===== END CRITICAL DEBUG =====
        ...
        message = (
            user_query
            or "Provide a comprehensive terrain analysis of this location including elevation, slope, and terrain classification."
        )
        ...
        try:
            if user_query or session_id:
                logger.info(
                    f"[MTN] [{request_id}] USING TERRAIN AGENT (with memory and tools)"
                )
                ...
                logger.info(
                    f"[MTN] [{request_id}] USING LEGACY ONE-SHOT AGENT (no memory)"
                )
            ...
            if (
                "403" in resp_text
                or "Public access is disabled" in resp_text
                or "private endpoint" in resp_text.lower()
            ):
                logger.warning(
                    f"[MTN] [{request_id}] Agent returned PE error, falling back to direct tools"
                )
                agent_result = None
        except asyncio.TimeoutError:
            agent_elapsed = time.time() - agent_start
            logger.error(
                f"[MTN] [{request_id}] [FAIL] AGENT TIMEOUT after {agent_elapsed:.1f}s"
            )
            raise HTTPException(
                status_code=504,
            ...
            logger.warning(
                f"[MTN] [{request_id}] Agent failed ({agent_err}), falling back to direct tools"
            )
            agent_result = None
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"Terrain analysis failed: {str(e)}"
        )
...
async def geoint_terrain_chat(body: TerrainChatRequest):
    ...
    try:
        ...
        logger.info(
            f"[MSG] [{request_id}] Screenshot: {'Yes (' + str(len(screenshot)) + ' chars)' if screenshot else 'No'}"
        )
        if screenshot:
            logger.info(
                f"[MSG] [{request_id}] Screenshot starts with: {screenshot[:60]}..."
            )
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"[MSG] [{request_id}] Agent returned PE error, falling back to direct tools"
                )
                result = None
        except Exception as agent_err:
            logger.warning(
                f"[MSG] [{request_id}] Agent failed ({agent_err}), falling back to direct tools"
            )
            result = None

        if result is None:
            logger.info(
                f"[MSG] [{request_id}] Using direct terrain tool fallback (PE lockdown)"
            )
            ...
            tool_results = {}
            for name, fn in [
                ("elevation", get_elevation_analysis),
                ("slope", get_slope_analysis),
                ("flat_areas", find_flat_areas),
                ("flood_risk", analyze_flood_risk),
            ]:
                try:
            ...
            try:
                ...
                synth_resp = await _terrain_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a terrain analysis expert. Synthesize the provided DEM/terrain tool data into a clear, comprehensive answer to the user's question.",
                        },
                ...
                response_text = (
                    tool_summary or "Terrain analysis completed with limited data."
                )
        ...
        logger.info(
            f"[MSG] [{request_id}] Tool calls: {len(result.get('tool_calls', []))}"
        )
...
async def geoint_vision_analysis(body: VisionRequest):
    ...
    try:
        ...
        logger.info(
            f"[EYE] [{request_id}] Tile URLs from request: {len(tile_urls_from_request)}"
        )
        logger.info(
            f"[EYE] [{request_id}] STAC items from request: {len(stac_items_from_request)}"
        )
        logger.info(
            f"[EYE] [{request_id}] Collection from request: {collection_from_request}"
        )
        if analysis_type:
        ...
        if not screenshot:
            raise HTTPException(
                status_code=400, detail="screenshot is required for vision analysis"
            )
        ...
        if not (-180 <= longitude <= 180):
            raise HTTPException(
                status_code=400, detail=f"Invalid longitude: {longitude}"
            )
        ...
        if not user_query:
            user_query = (
                "Analyze this location. What can you identify from the imagery?"
            )
        ...
        logger.info(
            f"[EYE] [{request_id}] Pin coordinates set: ({map_bounds.get('pin_lat')}, {map_bounds.get('pin_lng')})"
        )
        ...
        if stac_items_from_request:
            ...
            logger.info(
                f"[EYE] [{request_id}] Using {len(stac_items_from_request)} STAC items from request (with assets for NDVI)"
            )
            ...
            logger.info(
                f"[EYE] [{request_id}] Loaded session context: collections={collections_list}, stac_items={len(stac_items_from_session)}"
            )

        logger.info(
            f"[EYE] [{request_id}] Tile URLs for agent: {len(tile_urls_for_agent)}"
        )
        logger.info(
            f"[EYE] [{request_id}] STAC Items from session: {len(stac_items_from_session)}"
        )
        ...
        try:
            ...
            agent_elapsed = time.time() - agent_start
            logger.error(
                f"[EYE] [{request_id}] [FAIL] AGENT TIMEOUT after {agent_elapsed:.1f}s"
            )
            raise HTTPException(
                status_code=504,
        ...
        agent_elapsed = time.time() - agent_start
        logger.info(
            f"[EYE] [{request_id}] [OK] VISION AGENT COMPLETED ({agent_elapsed:.2f}s)"
        )
        ...
        if not response_text:
            ...
            raise HTTPException(
                status_code=500, detail="Vision agent returned empty response"
            )
...
async def geoint_vision_chat(body: VisionChatRequest):
    ...
    try:
        ...
        logger.info(f"[MSG] [{request_id}] Message: {message[:100]}")
        logger.info(
            f"[MSG] [{request_id}] Tile URLs from request: {len(tile_urls_from_request)}"
        )
        logger.info(
            f"[MSG] [{request_id}] STAC items from request: {len(stac_items_from_request)}"
        )

        if analysis_type:
            logger.info(
                f"[TARGET] [{request_id}] Analysis type hint from frontend: {analysis_type}"
            )
        ...
        if stac_items_from_request:
            for i, item in enumerate(stac_items_from_request[:2]):
                logger.info(
                    f"[MSG] [{request_id}] STAC item {i}: id={item.get('id', 'unknown')}, collection={item.get('collection', 'unknown')}, assets={list(item.get('assets', {}).keys())[:5]}"
                )
        logger.info(
            f"[MSG] [{request_id}] Collection from request: {collection_from_request}"
        )
        ...
        if latitude is not None and longitude is not None:
            ...
            logger.info(
                f"[MSG] [{request_id}] Pin coordinates set: ({latitude}, {longitude})"
            )
        ...
        final_stac_items = (
            stac_items_from_request
            if stac_items_from_request
            else stac_items_from_session
        )
        logger.info(
            f"[MSG] [{request_id}] Using {len(final_stac_items)} STAC items for analysis (request: {len(stac_items_from_request)}, session: {len(stac_items_from_session)})"
        )
        ...
        if not response_text:
            logger.warning(
                f"[MSG] [{request_id}] Empty response from vision agent in chat endpoint"
            )
            response_text = (
...
async def geoint_building_damage_analysis(body: BuildingDamageRequest):
    ...
    try:
        ...
        logger.info(
            f"Building Damage: screenshot={'yes' if screenshot else 'no'}, query='{user_query[:100]}'"
            if user_query
            else "Building Damage: no query"
        )
        ...
        _vision_client = get_llm_client(
            model=os.getenv("LLM_MODEL", "gpt-5"), vision=True
        )
        ...
        if screenshot:
            ...
            location_name = _reverse_geocode_cache.get(
                geo_key, f"({latitude:.4f}, {longitude:.4f})"
            )
            ...
            try:
                vision_response = await _vision_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a GEOINT Building Damage Assessment expert. Analyze the provided imagery and give structured damage assessments.",
                        },
                        {
                            "role": "user",
                            "content": [
                ...
                logger.info(
                    f"Building Damage: Screenshot analysis completed ({elapsed:.1f}s, {len(response_text)} chars)"
                )
                ...
                logger.warning(
                    f"Building Damage: Screenshot vision analysis failed: {vis_err}, falling back to Agent Service"
                )
                # Fall through to Agent Service path

        # ── PATH 2: No screenshot — use Agent Service to fetch and analyze imagery ──
        logger.info(
            "Building Damage: No screenshot, using Agent Service with tool-based imagery fetch"
        )
        ...
        try:
            ...
            if any(pat.lower() in resp_text.lower() for pat in _error_patterns):
                logger.warning(
                    f"Building damage agent returned error, falling back to direct tools: {resp_text[:200]}"
                )
                analysis_result = None  # trigger fallback
        except Exception as agent_err:
            err_str = str(agent_err)
            logger.warning(
                f"Building damage agent failed ({err_str}), falling back to direct tools"
            )
            analysis_result = None
        ...
        if analysis_result is None:
            ...
            from geoint.building_damage_tools import (
                _assess_damage_async,
                _classify_severity_async,
            )

            assess_result = await _assess_damage_async(
                ...
                try:
                    ...
                    vision_response = await _vision_client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "You are an expert in structural damage assessment from satellite imagery. Provide detailed, factual analysis.",
                            },
                            {
                                "role": "user",
                                "content": [
                                    {
                                        "type": "text",
                                        "text": f"Analyze this satellite image at ({latitude:.4f}, {longitude:.4f}) for building damage. "
                    ...
                    logger.info(
                        f"Building Damage: Screenshot analysis completed ({len(visual_analysis)} chars)"
                    )
                except Exception as vis_err:
                    logger.warning(
                        f"Building Damage: Screenshot analysis failed: {vis_err}"
                    )
            ...
            if visual_analysis:
                parts.append(
                    f"## Visual Analysis of Current Map View\n\n{visual_analysis}"
                )
            if tool_assessment:
            ...
            combined_response = (
                "\n\n".join(parts)
                if parts
                else "Building damage assessment completed. No significant damage indicators detected from available imagery."
            )
            ...
            analysis_result = {
                ...
                "tool_calls": [
                    {"tool": "assess_building_damage"},
                    {"tool": "classify_damage_severity"},
                ],
                "location": {"latitude": latitude, "longitude": longitude},
                "radius_miles": radius_miles,
                "timestamp": datetime.utcnow().isoformat(),
...
async def geoint_extreme_weather_analysis(body: ExtremeWeatherRequest):
    ...
    try:
        ...
        logger.info(
            f"[STORM] [{request_id}] Coordinates: ({latitude}, {longitude}), query: {user_query}"
        )
        ...
        is_overview_query = (
            ...
            or message
            == "Provide a comprehensive climate projection overview for this location."
        )

        if is_overview_query and not session_id:
            try:
                logger.info(
                    f"[STORM] [{request_id}] FAST PATH: Direct get_climate_overview call"
                )
                from geoint.extreme_weather_agent import _reverse_geocode_cache
                ...
                raw_result = await asyncio.wait_for(
                    loop.run_in_executor(
                        None, get_climate_overview, latitude, longitude, "ssp585", 2030
                    ),
                    timeout=120.0,
                )
                ...
                logger.info(
                    f"[STORM] [{request_id}] FAST PATH: Data fetched in {fast_elapsed:.1f}s"
                )
                ...
                if "error" not in overview_data:
                    ...
                    fmt_resp = await _fmt_client.chat.completions.create(
                        messages=[
                            {
                                "role": "system",
                                "content": "You are a climate analyst. Format the provided climate projection data into a clear, informative summary. Use the same style and formatting as your normal extreme weather analysis responses. Include all available metrics with their values and units.",
                            },
                            {
                                "role": "user",
                                "content": f"Format this climate projection data for {location_name} (SSP5-8.5, 2030) into a comprehensive summary:\n\n{raw_result}",
                            },
                        ],
                        max_completion_tokens=2000,
                        temperature=0.3,
                    ...
                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH COMPLETE in {total_elapsed:.1f}s (data: {fast_elapsed:.1f}s, format: {total_elapsed - fast_elapsed:.1f}s)"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"[STORM] [{request_id}] Pre-warming ssp245 STAC cache"
                        )
                    except Exception:
                        pass  # best-effort
                ...
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH timed out, falling back to agent"
                )
            except Exception as e:
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH failed ({e}), falling back to agent"
                )
        ...
        is_comparison_query = (
            (
                "compare" in _msg_lower
                and ("scenario" in _msg_lower or "ssp" in _msg_lower)
            )
            ...
            or (
                ...
                and (
                    "scenario" in _msg_lower
                    or "emission" in _msg_lower
                    or "climate" in _msg_lower
                )
            )
        )
        ...
        if is_comparison_query:
            try:
                logger.info(
                    f"[STORM] [{request_id}] FAST PATH COMPARE: Direct compare_climate_scenarios call"
                )
                from geoint.extreme_weather_agent import _reverse_geocode_cache
                ...
                fast_elapsed = time.time() - agent_start
                logger.info(
                    f"[STORM] [{request_id}] FAST PATH COMPARE: Data fetched in {fast_elapsed:.1f}s"
                )
                ...
                if "error" not in comparison_data:
                    ...
                    total_elapsed = time.time() - agent_start
                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH COMPARE COMPLETE in {total_elapsed:.1f}s (data: {fast_elapsed:.1f}s, format: {total_elapsed - fast_elapsed:.1f}s)"
                    )
                ...
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH COMPARE timed out, falling back to agent"
                )
            except Exception as e:
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH COMPARE failed ({e}), falling back to agent"
                )
        ...
        if matched_tool and not is_overview_query and not is_comparison_query:
            ...
            try:
                ...
                if (
                    "ssp245" in _msg_lower
                    or "ssp2-4.5" in _msg_lower
                    or "moderate" in _msg_lower
                ):
                    scenario = "ssp245"
                else:
                    scenario = "ssp585"
                ...
                # --------------------------------------------------------
                # TREND PATH: Fetch 2 years in parallel (2030 + 2070)
                # --------------------------------------------------------
                if is_trend_query:
                    trend_years = [2030, 2070]
                    if target_year not in trend_years and target_year != 2030:
                        trend_years[1] = target_year  # honour explicit year

                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH TREND: Parallel {tool_name} for years {trend_years}"
                    )

                    trend_tasks = [
                        loop.run_in_executor(
                            None, tool_fn, latitude, longitude, scenario, yr
                        )
                        for yr in trend_years
                    ]

                    trend_results_raw = await asyncio.wait_for(
                    ...
                    tool_calls_out = []
                    all_data_parts = []
                    for i, (yr, raw) in enumerate(zip(trend_years, trend_results_raw)):
                        if isinstance(raw, Exception):
                            logger.warning(
                                f"[STORM] [{request_id}] Trend year {yr} failed: {raw}"
                            )
                            continue
                        td = json.loads(raw)
                        if "error" not in td:
                            tool_calls_out.append({"tool": tool_name, "result": td})
                            all_data_parts.append(f"--- Year {yr} ---\n{raw}")

                    fast_elapsed = time.time() - agent_start
                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH TREND: Data for {len(all_data_parts)}/{len(trend_years)} years in {fast_elapsed:.1f}s"
                    )
                    ...
                    if all_data_parts:
                            ...
                            messages=[
                                {
                                    ...
                                    "content": (
                                        ...
                                        "You have projection data for multiple years from NASA NEX-GDDP-CMIP6. "
                                        "Compare the data across years to answer whether the variable is "
                                        "increasing, decreasing, or stable. Include actual values with units. "
                                        "Conclude with a Summary section that gives a clear, direct answer to the user's specific question, "
                                        "grounded in the data (e.g., 'Yes, peak daily rainfall is projected to "
                                        "increase from X mm/day in 2030 to Y mm/day in 2070')."
                                    ),
                                },
                                {
                                    "role": "user",
                                    "content": (
                                        f"Analyze {var_key} trend data for {location_name} "
                                        f"({scenario_desc}). "
                                        f"The user asked: '{user_query}'\n\n{combined_data}"
                                    ),
                                },
                        ...
                        total_elapsed = time.time() - agent_start
                        logger.info(
                            f"[STORM] [{request_id}] FAST PATH TREND COMPLETE in {total_elapsed:.1f}s"
                        )
                ...
                # --------------------------------------------------------
                # SINGLE-YEAR PATH (non-trend queries)
                # --------------------------------------------------------
                else:
                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH VARIABLE: Direct {tool_name} call"
                    )
                    raw_result = await asyncio.wait_for(
                        loop.run_in_executor(
                            None, tool_fn, latitude, longitude, scenario, target_year
                        ),
                        timeout=200.0,
                    )
                    ...
                    fast_elapsed = time.time() - agent_start
                    logger.info(
                        f"[STORM] [{request_id}] FAST PATH VARIABLE: {tool_name} data in {fast_elapsed:.1f}s"
                    )
                    ...
                    if "error" not in tool_data:
                        ...
                        total_elapsed = time.time() - agent_start
                        logger.info(
                            f"[STORM] [{request_id}] FAST PATH VARIABLE COMPLETE in {total_elapsed:.1f}s"
                        )
                        ...
                        return {
                            ...
                            "result": {
                                "analysis": formatted_response,
                                "tool_calls": [
                                    {"tool": tool_name, "result": tool_data}
                                ],
                                "message_count": 1,
                            },
                            "session_id": session_id,
                ...
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH VARIABLE timed out after {time.time() - agent_start:.0f}s, falling back to agent"
                )
            except Exception as e:
                logger.warning(
                    f"[STORM] [{request_id}] FAST PATH VARIABLE failed ({e}), falling back to agent"
                )
        ...
        try:
            ...
            logger.info(
                f"[STORM] [{request_id}] STANDARD PATH: elapsed={elapsed_so_far:.0f}s, budget={remaining_budget:.0f}s"
            )
            ...
            agent_elapsed = time.time() - agent_start
            logger.error(
                f"[STORM] [{request_id}] [FAIL] AGENT TIMEOUT after {agent_elapsed:.1f}s"
            )
            raise HTTPException(
                status_code=504,
        ...
        agent_elapsed = time.time() - agent_start
        logger.info(
            f"[STORM] [{request_id}] [OK] EXTREME WEATHER AGENT COMPLETED ({agent_elapsed:.2f}s)"
        )
        ...
        for i, tc in enumerate(tool_calls_list):
            ...
            if isinstance(tool_result, dict):
                has_error = "error" in tool_result or any(
                    "error" in str(v)
                    for v in tool_result.values()
                    if isinstance(v, (dict, str))
                )
                logger.info(
                    f"[STORM] [{request_id}]   Tool[{i}]: {tool_name} -> {'[FAIL] ERROR' if has_error else '[OK] OK'} (keys: {list(tool_result.keys())[:8]})"
                )
                if has_error:
                    logger.warning(
                        f"[STORM] [{request_id}]   Tool[{i}] error detail: {json.dumps(tool_result, default=str)[:500]}"
                    )
            else:
                logger.info(
                    f"[STORM] [{request_id}]   Tool[{i}]: {tool_name} -> {str(tool_result)[:200]}"
                )
        logger.info(
            f"[STORM] [{request_id}] Response preview ({len(response_text)} chars): {response_text[:300]}..."
        )
...
async def cmip6_diagnostic_test(
    ...
    from geoint.extreme_weather_tools import _sample_netcdf, _search_cmip6_items
    ...
    try:
        ...
        result["steps"].append(
            {"step": "STAC Search", "status": "error", "error": str(e)}
        )
        return result

    if not items:
        result["steps"].append(
            {"step": "NetCDF Sample", "status": "skipped", "reason": "No items found"}
        )
        return result
        ...
        result["steps"].append(
            {
                "step": "NetCDF Sample",
                "status": "error",
                "error": f"No href for {variable} asset",
            }
        )
        return result
    ...
    try:
        ...
        result["steps"].append(
            {"step": "NetCDF Sample", "status": "error", "error": str(e)}
        )
...
async def geoint_comparison_analysis(body: ComparisonRequest):
    ...
    try:
        ...
        logger.info(
            f"[CHART] [COMPARISON-ANALYSIS] mode={mode} query='{user_query}' before={before_date} after={after_date}"
        )
        ...
        if user_query and not (before_date and after_date):
            logger.info(
                "[SEARCH] QUERY MODE: Parsing query for comparison parameters..."
            )
            ...
            try:
                ...
                logger.info(
                    f"[OK] Comparison agent returned: status={result.get('status')}"
                )
                ...
                logger.warning(
                    f"[WARN] Comparison agent failed: {e}, falling back to direct STAC approach"
                )
                # --------------------------------------------------------
                ...
                try:
                    logger.info(
                        "[SEARCH] FALLBACK: Using direct STAC comparison approach"
                    )
                    # Step 1: Parse query with existing agents
                    if not semantic_translator:
                        raise RuntimeError("Semantic translator not initialized")

                    collections = await semantic_translator.collection_mapping_agent(
                        user_query
                    )
                    stac_query = await semantic_translator.build_stac_query_agent(
                        user_query, collections or []
                    )
                    bbox = stac_query.get("bbox")
                    location_name = stac_query.get("location_name", "Unknown location")
                    ...
                    if not bbox:
                        raise RuntimeError(
                            f"Could not resolve location from query: '{user_query}'"
                        )

                    datetime_result = (
                        await semantic_translator.datetime_translation_agent(
                            query=user_query,
                    ...
                    if (
                        not datetime_result
                        or "before" not in datetime_result
                        or "after" not in datetime_result
                    ):
                        raise RuntimeError(
                            "Could not extract before/after timeframes from query"
                        )
                    ...
                    before_result, after_result = await _asyncio.gather(
                        _execute_stac_search(
                            primary_collection, bbox, before_dt, limit=3
                        ),
                        _execute_stac_search(
                            primary_collection, bbox, after_dt, limit=3
                        ),
                    )
                    ...
                    fallback_result = {
                        ...
                        "before": {
                            ...
                            "best_scene_date": (
                                _get_scene_date(before_features[0])
                                if before_features
                                else None
                            ),
                        },
                        "after": {
                            ...
                            "best_scene_date": (
                                _get_scene_date(after_features[0])
                                if after_features
                                else None
                            ),
                        },
                    ...
                    logger.info(
                        f"[OK] FALLBACK comparison succeeded: {location_name}, {primary_collection}, before={len(before_features)} after={len(after_features)} scenes"
                    )
                    return {
                        "status": "success",
                        "type": "comparison",
                    ...
                    logger.error(
                        f"[FAIL] Fallback comparison also failed: {fallback_err}",
                        exc_info=True,
                    )
                    return {
                        "status": "error",
        ...
        # ================================================================
        logger.info("[SNAP] SCREENSHOT MODE: Using provided dates and screenshots...")
        ...
        logger.info(
            f"[SNAP] Before Screenshot: {'Provided' if before_screenshot else 'None'} ({len(before_screenshot) if before_screenshot else 0} chars)"
        )
        logger.info(
            f"[SNAP] After Screenshot: {'Provided' if after_screenshot else 'None'} ({len(after_screenshot) if after_screenshot else 0} chars)"
        )
        logger.info(
            f"[CHART] Before Metadata: {'Provided' if before_metadata else 'None'}"
        )
        logger.info(
            f"[CHART] After Metadata: {'Provided' if after_metadata else 'None'}"
        )
        logger.info(f"[MSG] User Query: {user_query}")
        ...
        if latitude is None or longitude is None or not before_date or not after_date:
            ...
            raise HTTPException(
                status_code=400,
                detail="Missing required parameters: latitude, longitude, before_date, after_date",
            )
        ...
        logger.info(
            f"     - before_screenshot_base64: {'Provided' if before_screenshot else 'None'}"
        )
        logger.info(
            f"     - after_screenshot_base64: {'Provided' if after_screenshot else 'None'}"
        )
...
async def geoint_orchestrator_endpoint(body: OrchestrateRequest):
    ...
    try:
        ...
        logger.info(
            f"Orchestrator endpoint: ({latitude}, {longitude}), modules={modules}"
        )
...
async def debug_location_resolver(location: str):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=500, detail=f"Location resolver debug failed: {str(e)}"
        )
