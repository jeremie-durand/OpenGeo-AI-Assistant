...
class TileSelector:
    ...
    def get_optimal_query_params(
        ...
        if user_datetime:
            ...
            if is_static_data:
                datetime_range = None
                logger.info("[MTN] Static dataset detected - no temporal filter needed")

            # Recent/Latest keywords -> Last 30 days
            elif any(
                term in query_lower
                for term in ["recent", "latest", "current", "now", "today"]
            ):
                start_date = (today - timedelta(days=30)).strftime("%Y-%m-%d")
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(
                    f"[TIME] 'Recent' detected -> Last 30 days: {datetime_range}"
                )

            # Last week
            elif any(
                term in query_lower for term in ["last week", "past week", "this week"]
            ):
                start_date = (today - timedelta(days=7)).strftime("%Y-%m-%d")
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(
                    f"[TIME] 'Last week' detected -> Last 7 days: {datetime_range}"
                )

            # Last month
            elif any(
                term in query_lower
                for term in ["last month", "past month", "this month"]
            ):
                start_date = (today - timedelta(days=30)).strftime("%Y-%m-%d")
                end_date = today.strftime("%Y-%m-%d")
                datetime_range = f"{start_date}/{end_date}"
                logger.info(
                    f"[TIME] 'Last month' detected -> Last 30 days: {datetime_range}"
                )
            ...
            # Last year
                ...
                datetime_range = f"{start_date}/{end_date}"
                logger.info(
                    f"[TIME] 'Last year' detected -> Last 365 days: {datetime_range}"
                )
            ...
            # DEFAULT: Last 60 days (good balance of recency + availability)
                ...
                datetime_range = f"{start_date}/{end_date}"
                logger.info(
                    f"[TIME] No date specified -> DEFAULT: Last 60 days: {datetime_range}"
                )
                logger.info("   [INFO] This ensures recent, relevant data by default")
        ...
        if is_optical:
            ...
            if wants_clouds and not wants_clear:
                ...
                logger.info("[CLOUD] User wants cloudy imagery - skipping cloud filter")
            else:
                # Default: <20% cloud cover for clear imagery
                cloud_threshold = 20

                # Extra strict for explicit "clear" requests
                if wants_clear or any(
                    term in query_lower for term in ["clear", "cloudless"]
                ):
                    cloud_threshold = 10
                    logger.info(
                        f"[CLOUD] 'Clear/low cloud' keyword detected -> cloud threshold: <{cloud_threshold}%"
                    )

                quality_filters["eo:cloud_cover"] = {"lt": cloud_threshold}
                logger.info(
                    f"[CLOUD] Cloud cover filter: <{cloud_threshold}% (optical imagery)"
                )

        # SAR imagery: No cloud filter needed (penetrates clouds)
        is_sar = any(col in collections for col in cls.SAR_COLLECTIONS)
        if is_sar:
            logger.info("[SIGNAL] SAR imagery detected - no cloud filter needed")
        ...
        if is_usda_cdl:
            ...
            logger.info(
                "[CROP] USDA CDL: Filtering to 'cropland' type items (required for render config)"
            )
        ...
        logger.info(
            f"[OK] Optimal query params generated: limit={query_limit}, "
    ...
    def select_best_tiles(
        ...
        if query_bbox and len(query_bbox) == 4:
            ...
            if area_degrees > 10:
                ...
                logger.info(
                    f"[GLOBE] Large area detected ({area_degrees:.1f} sq deg) - skipping date grouping for full spatial coverage"
                )
        ...
        if not skip_date_grouping:
            ...
            if tiles_by_date:
                ...
                best_date, best_date_tiles = cls._select_best_date_group(
                    tiles_by_date, query_bbox, weights
                )

                if best_date_tiles:
                    logger.info(
                        f"[DATE] Selected acquisition date: {best_date} ({len(best_date_tiles)} tiles)"
                    )
        ...
        if query_bbox:
            bbox_area = (query_bbox[2] - query_bbox[0]) * (
                query_bbox[3] - query_bbox[1]
            )
            if bbox_area > 25:  # Country-scale (e.g., Greece ~71 sq deg)
                logger.info(
                    f"[GLOBE] Country-scale area ({bbox_area:.1f} sq deg) -> using max_tiles={max_tiles}"
                )
            elif bbox_area > 0.25:  # Large region
                logger.info(
                    f"[MAP] Large area ({bbox_area:.1f} sq deg) -> using max_tiles={max_tiles}"
                )
        ...
        # =================================================================
        logger.info(
            f"[OK] Selected {len(selected)} tiles (from {len(features)} candidates)"
        )
        ...
        for i, tile in enumerate(selected[:3], 1):  # Log top 3
            ...
            logger.info(
                f"      [CHART] Scores: recency={breakdown['recency']:.1f}, "
    ...
    @classmethod
    def _group_tiles_by_acquisition_date(
        cls, features: List[Dict[str, Any]]
        ...
        logger.info(
            f"[CHART] Grouped {len(features)} tiles into {len(tiles_by_date)} acquisition dates"
        )
    ...
    def _select_best_date_group(
        ...
        for date_str, tiles in tiles_by_date.items():
            ...
            try:
                ...
                if days_old <= 7:
                    ...
                    recency_score = max(
                        10, 30 - (days_old // 365) * 5
                    )  # Decay over years
            except (ValueError, TypeError):
                recency_score = 50

            # Calculate coverage score based on unique grid cells (not raw tile count)
            # This avoids inflated scores when multiple items cover the same grid cell
            unique_grids = set()
            for tile in tiles:
                grid_id = cls._extract_grid_id(tile)
                if grid_id:
                    unique_grids.add(grid_id)
            unique_grid_count = len(unique_grids) if unique_grids else len(tiles)
            coverage_score = min(
                100, unique_grid_count * 20
            )  # 5+ unique grid cells = 100%
            ...
            avg_cloud_score = (
                sum(cloud_scores) / len(cloud_scores) if cloud_scores else 50
            )

            # Weighted total (prioritize recency, then coverage, then cloud)
            total_score = (
                (recency_score * 0.5) + (coverage_score * 0.3) + (avg_cloud_score * 0.2)
            )
        ...
        best = date_scores[0]
        logger.info(
            f"[DATE] Best acquisition date: {best['date']} (score={best['score']:.1f}, tiles={len(best['tiles'])}, recency={best['recency']}, cloud={best['cloud']:.1f})"
        )
    ...
    def _determine_scoring_weights(cls, query: Optional[str]) -> Dict[str, float]:
        ...
        # PRIORITY 2: User explicitly wants CLEAR/CLOUDLESS
            ...
            logger.info(
                "[SUN] User intent: CLEAR IMAGERY -> Prioritizing cloud cover (60%)"
            )
            weights = {
                "recency": 15.0,
        ...
        # PRIORITY 3: User explicitly wants HIGH RESOLUTION
            ...
            logger.info(
                "[SEARCH] User intent: HIGH RESOLUTION -> Prioritizing quality (50%)"
            )
            weights = {
                "recency": 20.0,
                "cloud_cover": 20.0,
        ...
        # PRIORITY 4: User wants FULL COVERAGE
            ...
            logger.info(
                "[MAP] User intent: FULL COVERAGE -> Prioritizing coverage (50%)"
            )
            weights = {
                "recency": 20.0,
                "cloud_cover": 15.0,
    ...
    @staticmethod
    def _extract_grid_id(feature: Dict[str, Any]) -> Optional[str]:
        """
        Extract the grid/tile ID from a STAC feature for deduplication.

        Works for Sentinel-2/HLS (MGRS tile), Landsat (WRS-2 path/row),
        and falls back to bbox-based grid ID.
        """
        ...
        # Fallback: bbox-based grid (round to 1 degree)
    ...
    def _calculate_optimal_tile_limit(
        cls, bbox: Optional[List[float]], collections: Optional[List[str]] = None
    ) -> int:
        """
        ...
        if not bbox or len(bbox) != 4:
            logger.info("[PIN] No bbox provided -> default limit: 100")
            return 100
        ...
        bbox_width_km = (
            bbox_width_deg * 111.0 * lat_correction
        )  # Width adjusted for latitude
        bbox_height_km = (
            bbox_height_deg * 111.0
        )  # Height (latitude doesn't affect N-S distance)
        bbox_area_km2 = bbox_width_km * bbox_height_km

        logger.info("[RULER] Query area dimensions:")
        logger.info(
            f"   Degrees: {bbox_width_deg:.4f}° × {bbox_height_deg:.4f}° = {bbox_area_deg:.4f} sq°"
        )
        logger.info(
            f"   Physical: {bbox_width_km:.1f}km × {bbox_height_km:.1f}km = {bbox_area_km2:.0f} km²"
        )
        ...
        if collections:
            ...
            for col in collections:
                ...
                if col in tile_sizes:
                    ...
                    if dynamic_size:
                        ...
                        logger.info(
                            f"   [SIGNAL] Dynamic query: {col} -> {dynamic_size}km tiles"
                        )
                    else:
                        # Ultimate fallback
                        collection_tile_sizes.append(100)
                        logger.info(
                            f"   [WARN] Unknown collection {col}, using 100km default"
                        )
            ...
            logger.info(
                f"[MAP] Typical tile size for {collections}: {typical_tile_size_km}km × {typical_tile_size_km}km"
            )
        else:
            logger.info(
                f"[MAP] Using default tile size: {typical_tile_size_km}km × {typical_tile_size_km}km"
            )
        ...
        logger.info("[CALC] Spatial calculation:")
        logger.info(f"   Query area: {bbox_area_km2:.0f} km²")
        logger.info(
            f"   Tile area: {tile_area_km2:.0f} km² ({typical_tile_size_km}×{typical_tile_size_km}km)"
        )
        logger.info(
            f"   Tiles needed (spatial with 30% margin): {tiles_needed_spatial}"
        )
        ...
        logger.info("[TARGET] Optimal tile limit calculation:")
        ...
        if final_limit == 50:
            logger.info("[PIN] TINY AREA: Using minimum limit (50)")
        elif final_limit < 150:
            logger.info("[PIN] SMALL AREA: Single tile or small mosaic")
        elif final_limit < 400:
            logger.info("[MAP] MEDIUM AREA: Multi-tile mosaic")
        elif final_limit < 800:
            logger.info("[MAP] LARGE AREA: Regional mosaic")
        else:
            logger.info("[MAP] VERY LARGE AREA: Using maximum limit (1000)")
    ...
    def _query_tile_size_from_stac(cls, collection_id: str) -> Optional[float]:
        ...
        try:
            ...
            if "grid:code" in summaries or "s2:mgrs_tile" in summaries:
                logger.debug("   Detected MGRS grid (Sentinel-2): ~100km tiles")
                return 100.0

            # Landsat uses WRS-2 path/row
            if "landsat:wrs_path" in summaries or "landsat:wrs_row" in summaries:
                logger.debug("   Detected WRS-2 grid (Landsat): ~185km scenes")
                return 185.0
            ...
            if "modis" in description or "modis" in keywords:
                logger.debug("   Detected MODIS: ~1200km tiles")
                return 1200.0

            if "dem" in keywords or "elevation" in keywords:
                logger.debug("   Detected DEM: ~1000km tiles")
                return 1000.0

            if "sar" in keywords or "sentinel-1" in description:
                logger.debug("   Detected SAR: ~250km tiles")
                return 250.0
            ...
            logger.debug("   Could not determine tile size from metadata")
            return None
