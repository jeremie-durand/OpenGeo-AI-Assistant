...
def _sample_cog_sync(
    ...
    try:
            ...
            with rasterio.open(signed_url) as src:
                ...
                if src.crs and str(src.crs) != "EPSG:4326":
                    ...
                    xs, ys = transform_coords(
                        "EPSG:4326", src.crs, [longitude], [latitude]
                    )
                    x, y = xs[0], ys[0]
                else:
                ...
                if row < 0 or row >= src.height or col < 0 or col >= src.width:
                    return {
                        "value": None,
                        "error": "Point outside raster pixel bounds",
                        "crs": crs,
                    }
                ...
                if nodata is not None:
                    if (math.isnan(nodata) and math.isnan(value)) or value == nodata:
                        return {
                            "value": None,
                            "error": "No data at this location (pixel masked)",
                            "nodata_value": nodata,
                            "crs": crs,
                            "reason": "nodata_mask",
                        }
                ...
                if math.isnan(value):
                    return {
                        "value": None,
                        "error": "No data at this location (NaN pixel)",
                        "crs": crs,
                        "reason": "nan_value",
                    }
                ...
                description = (
                    ...
                    if src.descriptions and len(src.descriptions) >= band
                    else None
                )
                return {
                    "value": value,
        ...
        if "409" in error_str and _retry_count < max_retries:
            ...
            time.sleep(delay)
            return _sample_cog_sync(
                cog_url, latitude, longitude, band, max_retries, _retry_count + 1
            )
...
def _compute_ndvi_sync(
    red_url: str, nir_url: str, bbox: Optional[List[float]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            signed_red = (
                pc.sign(red_url) if "blob.core.windows.net" in red_url else red_url
            )
            signed_nir = (
                pc.sign(nir_url) if "blob.core.windows.net" in nir_url else nir_url
            )
        except Exception:
        ...
        with rasterio.Env(**env_options):
            with rasterio.open(signed_red) as red_src, rasterio.open(
                signed_nir
            ) as nir_src:
                if bbox:
                ...
                moderate_veg = (
                    np.sum((ndvi_valid > 0.2) & (ndvi_valid <= 0.6))
                    / len(ndvi_valid)
                    * 100
                )
                sparse_veg = (
                    np.sum((ndvi_valid > 0) & (ndvi_valid <= 0.2))
                    / len(ndvi_valid)
                    * 100
                )
    ...
    try:
        ...
        if bounds:
            context_parts.append(
                f"Map location: ({bounds.get('center_lat', 'N/A')}, {bounds.get('center_lng', 'N/A')})"
            )
...
- If you can't see something clearly, say so"""
        ...
        _log_tool_call(
            "analyze_screenshot", {"question": question, "has_image": True}, result
        )
        return result
    ...
    try:
        ...
        if metric_type in ["elevation", "general"]:
            dem_items = [
                i
                for i in stac_items
                if "dem" in i.get("collection", "").lower()
                or "elevation" in i.get("collection", "").lower()
            ]
            if dem_items:
                item = dem_items[0]
                ...
                if item.get("bbox"):
                    bbox = item["bbox"]
                    results.append(
                        f"- Coverage: {bbox[0]:.2f}°W to {bbox[2]:.2f}°E, {bbox[1]:.2f}°S to {bbox[3]:.2f}°N"
                    )
        ...
        if metric_type in ["temperature", "sst", "general"]:
            ...
            sst_items = [
                i
                for i in stac_items
                if any(kw in i.get("collection", "").lower() for kw in sst_keywords)
            ]
            if not sst_items:
                sst_items = [
                    i
                    for i in stac_items
                    if "temperature" in i.get("collection", "").lower()
                ]
            ...
            if sst_items:
                ...
                if item.get("bbox"):
                    bbox = item["bbox"]
                    results.append(
                        f"- Coverage: {bbox[0]:.2f}° to {bbox[2]:.2f}°E, {bbox[1]:.2f}° to {bbox[3]:.2f}°N"
                    )
                results.append("\n**Temperature Information:**")
                results.append("- Data Unit: Kelvin (K)")
                results.append("- Typical Ocean Range: 270K to 310K (-3°C to 37°C)")
        ...
        if metric_type in ["ndvi", "general"]:
            ...
            optical_items = [
                i
                for i in stac_items
                if any(kw in i.get("collection", "").lower() for kw in optical_keywords)
            ]
            ...
            if optical_items:
                ...
                red_url = assets.get("B04", {}).get("href") or assets.get(
                    "red", {}
                ).get("href")
                nir_url = (
                    assets.get("B08", {}).get("href")
                    or assets.get("nir08", {}).get("href")
                    or assets.get("B8A", {}).get("href")
                )

                if red_url and nir_url:
                    logger.info(
                        f"[CHART] Computing NDVI from {item.get('collection')}..."
                    )
                    ...
                    if "error" in ndvi_stats:
                        ...
                        results.append(
                            f"- Valid Pixels: {ndvi_stats['valid_pixels']:,} / {ndvi_stats['total_pixels']:,}"
                        )
                        ...
                        if "classification" in ndvi_stats:
                            ...
                            results.append(
                                f"- Dense Vegetation (NDVI > 0.6): {cls['dense_vegetation']:.1f}%"
                            )
                            results.append(
                                f"- Moderate Vegetation (0.2-0.6): {cls['moderate_vegetation']:.1f}%"
                            )
                            results.append(
                                f"- Sparse Vegetation (0-0.2): {cls['sparse_vegetation']:.1f}%"
                            )
                            results.append(
                                f"- Non-Vegetation (≤ 0): {cls['non_vegetation']:.1f}%"
                            )
        ...
        if metric_type == "general" and stac_items:
            ...
            for i, item in enumerate(stac_items[:5]):
                props = item.get("properties", {})
                dt = (
                    props.get("datetime", "unknown")[:10]
                    if props.get("datetime")
                    else "unknown"
                )
                cloud = props.get("eo:cloud_cover", "N/A")
                cloud_str = (
                    f"{cloud:.1f}%" if isinstance(cloud, (int, float)) else cloud
                )
                results.append(
                    f"  {i + 1}. {item.get('id', 'unknown')} ({dt}, cloud: {cloud_str})"
                )

        if not results:
            return (
                f"No {metric_type} data available in loaded collections: {collections}"
            )
...
def analyze_vegetation(analysis_type: str = "general") -> str:
    ...
    try:
        ...
        veg_products = {
            ...
            "modis-15a2h-061": {
                "name": "MODIS LAI/FPAR 8-Day",
                "metrics": ["LAI", "FPAR"],
                "resolution": "500m",
            },
            "modis-17a2h-061": {
                "name": "MODIS GPP 8-Day",
                "metrics": ["GPP", "PSN"],
                "resolution": "500m",
            },
            "modis-17a3hgf-061": {
                "name": "MODIS NPP Yearly",
                "metrics": ["NPP"],
                "resolution": "500m",
            },
        }
        ...
        if optical_items and analysis_type in ["ndvi", "general"]:
            ...
            for item in optical_items[:3]:
                ...
                assets = item.get("assets", {})
                product_info = next(
                    (info for key, info in optical_products.items() if key in coll),
                    None,
                )
                if product_info:
                    results.append(
                        f"**{product_info['name']}:** Item: {item.get('id')}"
                    )
                    if props.get("datetime"):
                        results.append(
                            f"- Date: {props['datetime'][:10]}, Resolution: {product_info['resolution']}"
                        )
                    has_red = product_info["red"] in assets
                    has_nir = product_info["nir"] in assets
                    if has_red and has_nir:
                        results.append(
                            f"- [OK] RED ({product_info['red']}) and NIR ({product_info['nir']}) available for NDVI"
                        )
            ...
            results.append(
                "**NDVI Interpretation:** -1->0: Water/bare | 0->0.2: Sparse | 0.2->0.5: Moderate | 0.5->1.0: Dense\n"
            )
            return "\n".join(results)
        ...
        if veg_items:
            ...
            for item in veg_items[:3]:
                ...
                props = item.get("properties", {})
                product_info = next(
                    (info for key, info in veg_products.items() if key in coll), None
                )
                if product_info:
                    results.append(f"**{product_info['name']}:**")
                    results.append(
                        f"- Item: {item.get('id')}, Resolution: {product_info['resolution']}"
                    )
                    if props.get("datetime"):
                        results.append(f"- Date: {props['datetime'][:10]}")
        ...
        if not results:
            results.append(
                "No vegetation data loaded. Use modis-13a1-061 for NDVI or optical imagery (HLS, Sentinel-2)."
            )
...
def analyze_fire(analysis_type: str = "general") -> str:
    ...
    fire_products = {
        "modis-14a1-061": {
            "name": "MODIS Thermal Anomalies Daily (1km)",
            "type": "active_fire",
        },
        "modis-14a2-061": {
            "name": "MODIS Thermal Anomalies 8-Day (1km)",
            "type": "active_fire",
        },
    ...
    fire_items = [
        i
        for i in stac_items
        if any(fp in i.get("collection", "").lower() for fp in fire_products)
    ]
    results = []
    ...
    if fire_items:
        ...
        for item in fire_items[:3]:
            ...
            props = item.get("properties", {})
            product_info = next(
                (info for key, info in fire_products.items() if key in coll), None
            )
            if product_info:
                results.append(f"**{product_info['name']}:**")
                ...
                if props.get("datetime"):
                    results.append(f"- Date: {props['datetime'][:10]}")
                results.append(
                    f"- Detection Type: {product_info['type'].replace('_', ' ').title()}"
                )
                if product_info["type"] == "active_fire":
                    results.append("- Confidence levels: Low/Nominal/High")
                    results.append(
                        "- Fire Radiative Power (FRP) in MW indicates intensity"
                    )
...
def analyze_land_cover(analysis_type: str = "general") -> str:
    ...
    lc_items = [
        i
        for i in stac_items
        if any(lp in i.get("collection", "").lower() for lp in lc_products)
    ]
    results = []
    ...
    if lc_items:
        ...
        for item in lc_items[:3]:
            ...
            props = item.get("properties", {})
            product_info = next(
                (info for key, info in lc_products.items() if key in coll), None
            )
            if product_info:
                results.append(f"**{product_info['name']}:**")
                results.append(
                ...
                if props.get("datetime"):
                    results.append(f"- Date: {props['datetime'][:10]}")
                results.append(
                    "- Classes: Water, Trees, Grassland, Cropland, Built-up, Bare, Wetlands"
                )
                results.append("")
    else:
...
def analyze_snow(analysis_type: str = "general") -> str:
    ...
    snow_products = {
        "modis-10a1-061": {
            "name": "MODIS Snow Cover Daily (500m)",
            "temporal": "daily",
        },
        "modis-10a2-061": {
            "name": "MODIS Snow Cover 8-Day (500m)",
            "temporal": "8-day",
        },
    }
    ...
    snow_items = [
        i
        for i in stac_items
        if any(sp in i.get("collection", "").lower() for sp in snow_products)
    ]
    results = []
    ...
    if snow_items:
        ...
        for item in snow_items[:3]:
            ...
            props = item.get("properties", {})
            product_info = next(
                (info for key, info in snow_products.items() if key in coll), None
            )
            if product_info:
                results.append(f"**{product_info['name']}:**")
                results.append(f"- Item: {item.get('id')}")
                if props.get("datetime"):
                    results.append(f"- Date: {props['datetime'][:10]}")
                results.append("- NDSI Snow Cover: 0-100%")
                results.append(
                    "- 0-10%: Snow-free | 10-50%: Partial | 50-100%: Significant coverage"
                )
                results.append("")
    else:
...
def analyze_sar(analysis_type: str = "general") -> str:
    ...
    sar_items = [
        i
        for i in stac_items
        if any(sp in i.get("collection", "").lower() for sp in sar_products)
    ]
    results = []
    ...
    if sar_items:
        ...
        for item in sar_items[:3]:
            ...
            assets = item.get("assets", {})
            product_info = next(
                (info for key, info in sar_products.items() if key in coll), None
            )
            if product_info:
                results.append(f"**{product_info['name']}:**")
                results.append(
                    f"- Item: {item.get('id')}, Type: {product_info['type']}"
                )
                if props.get("datetime"):
                    results.append(f"- Date: {props['datetime'][:10]}")
                ...
                if pols:
                    results.append(
                        f"- Polarizations: {', '.join(p.upper() for p in pols)}"
                    )
                results.append(
                    "- Dark areas = smooth/water | Bright = rough/urban/forest"
                )
...
def analyze_water(analysis_type: str = "general") -> str:
    ...
    if water_items:
        ...
        for item in water_items[:3]:
            results.append("**JRC Global Surface Water:**")
            results.append(f"- Item: {item.get('id')}")
            results.append("- Resolution: 30m, Period: 1984-present")
        ...
        for item in sar_items[:3]:
            results.append("**Sentinel-1 Water Detection:**")
            results.append(f"- Item: {item.get('id')}")
        ...
        results.append(
            "No water data loaded. Try jrc-gsw or sentinel-1-rtc for flood detection."
        )
...
def analyze_biomass(analysis_type: str = "general") -> str:
    ...
    biomass_items = [
        i
        for i in stac_items
        if any(k in i.get("collection", "").lower() for k in ["biomass", "chloris"])
    ]
    results = []
    ...
    if biomass_items:
        ...
        for item in biomass_items[:3]:
            results.append(f"- Item: {item.get('id')}")
            results.append("- Unit: Mg/ha (tonnes per hectare)")
            results.append(
                "- 0-50: Grassland | 50-150: Woodland | 150-300: Dense forest | 300+: Tropical rainforest"
            )
...
def sample_raster_value(data_type: str = "auto") -> str:
    ...
    try:
            ...
            for item in stac_items:
                ...
                if (
                    ...
                    asset_keys.append(
                        "data"
                        if "data" in assets
                        else (list(assets.keys())[0] if assets else "data")
                    )
            ...
            for item in stac_items:
                ...
                if any(kw in coll for kw in optical_keywords):
                    ...
                    red_key = (
                        "B04"
                        if "B04" in assets
                        else ("red" if "red" in assets else None)
                    )
                    nir_key = None
        ...
        # MTBS Burn Severity
        if (
            data_type in ["burn", "severity", "mtbs", "fire", "auto"]
            and not target_items
        ):
            for item in stac_items:
                coll = item.get("collection", "").lower()
                if "mtbs" in coll or "burn" in coll:
                    ...
                    target_items.append(item)
                    asset_keys.append(
                        "burn-severity" if "burn-severity" in assets else "data"
                    )
                    value_transforms.append(
                        {
            ...
            for item in stac_items:
                ...
                if "modis-14" in coll or "fire" in coll:
                    ...
                    if "MaxFRP" in assets:
                        ...
                        value_transforms.append(
                            {
                                "name": "Fire Radiative Power (MaxFRP)",
                                "unit_raw": "MW",
                                "unit_display": "MW",
        ...
        if data_type in ["water", "occurrence", "jrc", "auto"] and not target_items:
            for item in stac_items:
                ...
                if (
                    "jrc" in coll
                    or "gsw" in coll
                    or "surface-water" in coll
                    or "occurrence" in assets
                ):
                    asset_key = next(
                        (
                            k
                            for k in ["occurrence", "extent", "seasonality", "data"]
                            if k in assets
                        ),
                        "data",
                    )
        ...
        if data_type in ["snow", "ice", "auto"] and not target_items:
            for item in stac_items:
                ...
                if "modis-10" in coll or "snow" in coll:
                    ...
                    target_items.append(item)
                    asset_keys.append(
                        "NDSI_Snow_Cover" if "NDSI_Snow_Cover" in assets else "data"
                    )
                    value_transforms.append(
                        {
                            "name": "Snow Cover",
        ...
        if data_type in ["landcover", "cdl", "crop", "auto"] and not target_items:
            for item in stac_items:
                coll = item.get("collection", "").lower()
                if "cdl" in coll or "cropland" in coll or "land-cover" in coll:
                    assets = item.get("assets", {})
                    target_items.append(item)
                    asset_keys.append(
                        "data"
                    ...
                    value_transforms.append(
                        {
                            ...
                            "unit_display": "Mg/ha",
                            "transform": lambda v, a=pixel_area_ha: (
                                round(v / a, 1) if v else v
                            ),
                            "valid_range": (0, 500),
                        }
                    )
        ...
        if (
            data_type in ["vegetation", "lai", "gpp", "npp", "auto"]
            and not target_items
        ):
            for item in stac_items:
                coll = item.get("collection", "").lower()
        ...
        # SAR/Radar
        if (
            data_type in ["sar", "radar", "backscatter", "water", "auto"]
            and not target_items
        ):
            for item in stac_items:
                coll = item.get("collection", "").lower()
                            ...
                            value_transforms.append(
                                {
                                    ...
                                    "unit_display": "dB",
                                    "transform": lambda v: (
                                        10 * math.log10(v) if v and v > 0 else v
                                    ),
                                    "valid_range": (-30, 10),
                                }
                            )
            ...
            for item in stac_items:
                ...
                if "nex-gddp" in coll or "cmip6" in coll:
                    ...
                    if not chosen_var and assets:
                        chosen_var = next(
                            (k for k in assets if k in climate_var_info), None
                        )
                    if chosen_var:
                        ...
                        if var_info["kelvin"]:
                            ...
                            value_transforms.append(
                                {
                                    "name": var_info["name"],
                                    "unit_raw": var_info["unit"],
                                    "unit_display": var_info["display_unit"],
                                    "transform": lambda v: round(v, 2) if v else v,
                                    "valid_range": None,
                                    "is_netcdf": True,
                                }
                            )
        ...
        if not target_items and stac_items:
            for item in stac_items:
                ...
                for asset_key, asset_info in assets.items():
                    ...
                    is_raster = asset_type.startswith(
                        "image/tiff"
                    ) or asset_type.startswith("application/x-geotiff")
                    is_common = asset_key.lower() in [
                        "data",
                        "visual",
                        "default",
                        "occurrence",
        ...
        items_in = [
            (it, ak, tf)
            for it, ak, tf in zip(target_items, asset_keys, value_transforms)
            if point_in_bbox(lat, lng, it.get("bbox"))
        ]
        items_out = [
            (it, ak, tf)
            for it, ak, tf in zip(target_items, asset_keys, value_transforms)
            if not point_in_bbox(lat, lng, it.get("bbox"))
        ]
        ...
        if not items_in and items_out:
            ...
            if collection_id:
                try:
                    ...
                    is_optical_coll = any(
                        kw in collection_id.lower()
                        for kw in ["sentinel-2", "landsat", "hls", "s30", "l30"]
                    )
                    search_body = {
                            ...
                            for feature in resp.json().get("features", []):
                                if point_in_bbox(lat, lng, feature.get("bbox")):
                                    ...
                                    if is_ndvi_sampling:
                                        ...
                                        "dem" in collection_id.lower()
                                        or "cop-dem" in collection_id.lower()
                                    ):
                                        ak = "data"
                                        tf_info = {
                                            "name": "Elevation",
        ...
        for i, (item, asset_key, transform_info) in enumerate(sorted_data[:8]):
            ...
            if transform_info.get("is_ndvi") and isinstance(asset_key, tuple):
                ...
                if red_url and nir_url:
                    ...
                    if (
                        red_result.get("error")
                        and "outside" in str(red_result["error"]).lower()
                    ):
                        outside_tiles.append({"id": tile_id_short, "date": tile_date})
                        continue
                    if (
                        nir_result.get("error")
                        and "outside" in str(nir_result["error"]).lower()
                    ):
                        outside_tiles.append({"id": tile_id_short, "date": tile_date})
                        continue
                    if (
                        red_result.get("error")
                        and "no data" in str(red_result["error"]).lower()
                    ):
                        nodata_tiles.append(
                            {
                                "id": tile_id_short,
                                "date": tile_date,
                                "cloud_cover": tile_cloud,
                            }
                        )
                    ...
                    if (
                        nir_result.get("error")
                        and "no data" in str(nir_result["error"]).lower()
                    ):
                        nodata_tiles.append(
                            {
                                "id": tile_id_short,
                    ...
                    if (
                        red_result.get("value") is not None
                        and nir_result.get("value") is not None
                    ):
                        red_val = float(red_result["value"])
                        nir_val = float(nir_result["value"])
                        if (nir_val + red_val) != 0:
                            ndvi = max(
                                -1, min(1, (nir_val - red_val) / (nir_val + red_val))
                            )
                            results.append(f"**NDVI at Pin ({collection}):**")
                            results.append(
                                f"- RED ({red_key}): {red_val:.0f}, NIR ({nir_key}): {nir_val:.0f}"
                            )
            ...
            if transform_info.get("is_netcdf"):
                nc_href = (
                    assets.get(asset_key, {}).get("href", "")
                    if isinstance(assets.get(asset_key), dict)
                    else ""
                )
                if nc_href:
                    try:
                        ...
                        signed_url = (
                            pc.sign(nc_href)
                            if "blob.core.windows.net" in nc_href
                            else nc_href
                        )
                        ...
                        with rasterio.Env(**env_options):
                            with rasterio.open(netcdf_path) as src:
                                ...
                                try:
                                    ...
                                    results.append(
                                        f"**{transform_info['name']} ({collection}):** Coordinate outside grid"
                                    )
                                    continue
                                if (
                                    row < 0
                                    ...
                                    outside_tiles.append(
                                        {"id": tile_id_short, "date": tile_date}
                                    )
                                    continue
                                ...
                                if nodata is not None and raw_value == nodata:
                                    ...
                                    continue
                                try:
                                    display_value = transform_info["transform"](
                                        raw_value
                                    )
                                except Exception:
                                    display_value = raw_value
                                model_scenario = (
                                    tile_id_short  # e.g. "ACCESS-CM2.ssp585.2050"
                                )
                                results.append(
                                    f"**{transform_info['name']} ({collection}):**"
                                )
                                results.append(
                                    f"- Raw value: {raw_value:.4f} {transform_info['unit_raw']}"
                                )
                                results.append(
                                    f"- **Converted: {display_value} {transform_info['unit_display']}**"
                                )
                                results.append(f"- Model/Scenario: {model_scenario}")
                                results.append("- Grid resolution: 0.25° × 0.25°")
                                results.append(
                                    f"- Band sampled: {last_band} (last day in file)"
                                )
                                if tile_date:
                        ...
                        logger.warning(
                            f"[WARN] NetCDF sampling failed for {collection}/{asset_key}: {e}"
                        )
                        ...
                        results.append(
                            "- [WARN] Direct NetCDF sampling not available (data is not COG-optimized)"
                        )
                        results.append(
                            f"- Variable: {asset_key} ({transform_info['name']})"
                        )
                        results.append(
                            f"- Model: {model_name}, Scenario: {scenario}, Year: {year}"
                        )
                        results.append(
                            f"- Unit: {transform_info['unit_raw']} -> {transform_info['unit_display']}"
                        )
                        results.append("- Resolution: 0.25° × 0.25° global grid")
                        results.append(
                            "- This is a CMIP6 climate **projection**, not observed data"
                        )
                        error_tiles.append(
                            {
                                "id": tile_id_short,
                    ...
                    results.append(
                        f"**{transform_info['name']}:** No asset URL for variable '{asset_key}'"
                    )
                results.append("")
                continue
            ...
            if not cog_url:
                for fk in ["data", "visual", "default"]:
                    if fk in assets:
                        ai = assets[fk]
                        if ai.get("type", "").startswith("image/tiff") or ai.get(
                            "href", ""
                        ).endswith(".tif"):
                            cog_url = ai.get("href")
                            break
            ...
            if sample_result.get("error"):
                ...
                logger.warning(
                    f"[WARN] COG sampling error for {tile_id_short}: {sample_result['error']}"
                )
                ...
                if class_labels:
                    ...
                    results.append(
                        f"- Class: **{int_val}** — {class_labels.get(int_val, f'Unknown class {int_val}')}"
                    )
                else:
                    if transform_info["unit_raw"] != transform_info["unit_display"]:
                        results.append(
                            f"- Raw: {raw_value:.2f} {transform_info['unit_raw']}"
                        )
                        results.append(
                            f"- Converted: **{display_value:.2f} {transform_info['unit_display']}**"
                        )
                    else:
                        results.append(
                            f"- Value: **{display_value:.2f} {transform_info['unit_display']}**"
                        )
                    ...
                    if "water" in name_lower or "occurrence" in name_lower:
                        pct = float(raw_value)
                        if pct == 0:
                            results.append("- [DESERT] Never water")
                        elif pct < 25:
                            results.append(f"- [DROP] Rarely water ({pct:.0f}%)")
                        elif pct < 75:
                            results.append(f"- [WAVE] Often water ({pct:.0f}%)")
                        else:
                            results.append(
                                f"- [WAVE][WAVE] Permanent water ({pct:.0f}%)"
                            )
                    elif "snow" in name_lower:
                        pct = float(raw_value)
                        if pct == 0:
                            results.append("- [SUN] No snow")
                        elif pct < 25:
                            results.append(f"- [SNOW] Light snow ({pct:.0f}%)")
                        elif pct < 75:
                            results.append(f"- [SNOW][SNOW] Moderate snow ({pct:.0f}%)")
                        else:
                            results.append(
                                f"- [SNOW][SNOW][SNOW] Heavy snow ({pct:.0f}%)"
                            )
        ...
        if sampled_count == 0 and stac_items:
            ...
            if collection_id and (is_ndvi_sampling or is_optical_coll):
                try:
                    ...
                    logger.info(
                        f"[PIN] Clear-sky fallback search: collection={collection_id}, bbox={pin_bbox}"
                    )
                    ...
                    with httpx.Client(timeout=30) as http_client:
                        ...
                        if resp.status_code == 200:
                            ...
                            logger.info(
                                f"[PIN] Clear-sky fallback found {len(features)} candidates"
                            )
                            ...
                            for feature in features:
                                ...
                                if is_ndvi_sampling:
                                    ...
                                    red_key = (
                                        "B04"
                                        if "B04" in f_assets
                                        else ("red" if "red" in f_assets else None)
                                    )
                                    nir_key = None
                                    for candidate in [
                                        "B08",
                                    ...
                                    if red_result.get("error") or nir_result.get(
                                        "error"
                                    ):
                                        logger.info(
                                            f"[PIN] Fallback tile {f_id_short} also masked: red={red_result.get('error')}, nir={nir_result.get('error')}"
                                        )
                                        continue
                                    ...
                                    if (
                                        ...
                                        if (nir_val + red_val) != 0:
                                            ...
                                            if f_cloud is not None:
                                                results.append(
                                                    f"- Cloud cover: {f_cloud:.0f}%"
                                                )
                                            results.append(
                                                "- *(Used a different date because the loaded tiles were cloud-masked at this pin)*"
                                            )
                                            sampled_count += 1
                                else:
            ...
            if nodata_tiles:
                avg_cloud = (
                    sum(
                        ...
                        if t.get("cloud_cover") is not None
                    )
                    / max(1, len(tile_cloud_covers))
                    if tile_cloud_covers
                    else None
                )

                dates_str = ", ".join(t["date"] for t in nodata_tiles if t.get("date"))
                results.append(
                    f"- **{len(nodata_tiles)} tile{'s' if len(nodata_tiles) != 1 else ''} had masked/empty pixels** at this pin location"
                )

                # Determine if it's optical data (cloud-maskable) vs other
                is_optical = any(
                    kw in " ".join(collections).lower()
                    for kw in ["sentinel-2", "landsat", "hls", "s30", "l30", "naip"]
                )

                if is_optical and avg_cloud is not None and avg_cloud > 20:
                    results.append(
                        f"- **Likely cause: Cloud cover** — the {len(nodata_tiles)} available tile{'s' if len(nodata_tiles) != 1 else ''} had an average of {avg_cloud:.0f}% cloud cover"
                    )
                    results.append(
                        f"  The pixel at your pin ({lat:.4f}°, {lng:.4f}°) falls in a cloud-masked area"
                    )
                    results.append(f"  Dates attempted: {dates_str}")
                    results.append("\n**What to try:**")
                    results.append(
                        "- [SYNC] Move the pin to a nearby area where imagery is visible on the map"
                    )
                    results.append(
                        "- [DATE] Try a different date range with clearer skies"
                    )
                elif is_optical:
                    results.append(
                        "- **Likely cause: Cloud/shadow/snow mask** — the L2A processing pipeline masked this pixel"
                    )
                    results.append(
                        "  Even with low overall cloud cover, your specific pin location may be under a cloud or shadow"
                    )
                    if dates_str:
                        results.append(f"  Dates attempted: {dates_str}")
                    results.append("\n**What to try:**")
                    results.append(
                        "- [SYNC] Move the pin to a nearby area where data is visually rendered on the map"
                    )
                    results.append("- [DATE] Request a different date range")
                else:
                    results.append(
                        f"- **Cause: NoData mask** — the pixel at ({lat:.4f}°, {lng:.4f}°) has no valid data in this collection"
                    )
                    if dates_str:
                        results.append(f"  Dates attempted: {dates_str}")
                    results.append("\n**What to try:**")
                    results.append(
                        "- [SYNC] Move the pin to an area where data is displayed on the map"
                    )

            if outside_tiles:
                results.append(
                    f"- **{len(outside_tiles)} tile{'s' if len(outside_tiles) != 1 else ''} did not cover this pin location** (pin is outside their spatial extent)"
                )

            if error_tiles:
                error_types = set(t.get("error", "unknown")[:50] for t in error_tiles)
                results.append(
                    f"- **{len(error_tiles)} tile{'s' if len(error_tiles) != 1 else ''} had access errors:** {'; '.join(error_types)}"
                )
                results.append(
                    "  This may be a temporary Planetary Computer issue — try again in a moment"
                )
        ...
        _log_tool_call(
            "sample_raster_value",
            {
                ...
                "nodata": len(nodata_tiles),
                "outside": len(outside_tiles),
                "errors": len(error_tiles),
            },
...
def query_knowledge(question: str) -> str:
    ...
    try:
        ...
        if bounds:
            context_parts.append(
                f"User is viewing: ({bounds.get('center_lat', 'N/A')}, {bounds.get('center_lng', 'N/A')})"
            )
        collections = ctx.get("loaded_collections", [])
        if collections:
...
def identify_features(feature_type: str) -> str:
    ...
    try:
        ...
        bounds = ctx.get("map_bounds", {})
        location_hint = (
            f"Approximate location: ({bounds.get('center_lat', 'N/A')}, {bounds.get('center_lng', 'N/A')})"
            if bounds
            else ""
        )
...
def _select_collection_for_analysis(analysis_focus: str) -> str:
    ...
    mapping = {
        ...
        "reflectance": "hls",
        "urban development": "sentinel-2-l2a",
        "urban": "sentinel-2-l2a",
        "water levels": "jrc-gsw",
        "snow cover": "modis-snow",
        "snow": "modis-snow",
        "fire": "modis-fire",
        "general": "sentinel-2-l2a",
    }
...
def _execute_stac_query_sync(
    collection: str, bbox: List[float], datetime_range: str, limit: int = 5
) -> Dict[str, Any]:
...
def _sample_reflectance_at_point(
    features: List[Dict], lat: float, lng: float, bands: Optional[List[str]] = None
) -> Dict[str, Any]:
...
def compare_temporal(
    ...
    time_period_1: str,
    time_period_2: str,
    analysis_focus: str = "surface reflectance",
) -> str:
    ...
    """
    logger.info(
        f"[WAIT] compare_temporal(location='{location}', t1='{time_period_1}', t2='{time_period_2}')"
    )
    ...
    try:
        ...
        if center_lat and center_lng and features_1 and features_2:
            bands = (
                ["B04", "B08"]
                if "ndvi" in analysis_focus.lower()
            ...
            if s1.get("values") and s2.get("values"):
                ...
                if "B04" in common and "B08" in common:
                    r1, n1 = (
                        s1["values"]["B04"]["reflectance"],
                        s1["values"]["B08"]["reflectance"],
                    )
                    r2, n2 = (
                        s2["values"]["B04"]["reflectance"],
                        s2["values"]["B08"]["reflectance"],
                    )
            ...
            try:
                resp = client.chat.completions.create(
                    messages=[
                        {
                            "role": "system",
                            "content": f"""Geospatial analyst comparing imagery.
...
Describe expected changes, seasonal effects, and what to examine.""",
                        ...
                        {
                            "role": "user",
                            "content": f"Compare {analysis_focus} in {location} between {time_period_1} and {time_period_2}.",
                        },
                    ],
        ...
        _log_tool_call(
            "compare_temporal",
            {"location": location, "t1": time_period_1, "t2": time_period_2},
            result[:200],
        )
        return result
