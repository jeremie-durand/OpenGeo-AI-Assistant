...
class EnhancedLocationResolver:
    ...
    STORED_LOCATIONS = {
        ...
        "kathmandu nepal": [85.20, 27.60, 85.45, 27.80],  # Alias
        ...
        "houston texas": [-95.82, 29.52, -95.01, 30.11],  # Alias
        ...
        "denver colorado": [-105.11, 39.61, -104.60, 39.91],  # Alias
        ...
        "new orleans louisiana": [-90.14, 29.87, -89.63, 30.20],  # Alias
        ...
        "paradise california": [-121.72, 39.66, -121.52, 39.86],  # Alias
        ...
        "mount rainier washington": [-121.88, 46.72, -121.60, 46.93],  # Alias
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"[OK] Enhanced Location Resolver initialized with {len(self.STORED_LOCATIONS)} stored global locations (countries, cities, landmarks, natural wonders) + dynamic API resolution"
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_result:
                health_status["nominatim"] = {
                    "status": "healthy",
                    "message": "OSM fallback available",
                }
            else:
                health_status["nominatim"] = {
                    "status": "degraded",
                    "message": "OSM available but test failed",
                }
        except Exception as e:
            health_status["nominatim"] = {
                "status": "unhealthy",
                "message": f"OSM unavailable: {str(e)}",
            }
    ...
    async def resolve_location_to_bbox(
        ...
        """
        self.logger.info(
            f"[SEARCH] Resolving location: '{location_name}' (type: {location_type})"
        )
        ...
        for loc_variant in [location_lower, location_normalized]:
            if loc_variant in self.STORED_LOCATIONS:
                ...
                self.logger.info(
                    f"[OK] Resolved from hardcoded US locations: '{location_name}' -> {bbox}"
                )
            ...
            for descriptor in descriptors_to_strip:
                if loc_variant.endswith(descriptor):
                    ...
                    if stripped_name in self.STORED_LOCATIONS:
                        ...
                        self.logger.info(
                            f"[OK] Resolved from hardcoded locations (stripped '{descriptor}'): '{location_name}' -> '{stripped_name}' -> {bbox}"
                        )
                        self.cache.set(location_name, location_type, bbox)
                        return bbox
        ...
        for loc_variant in [location_lower, location_normalized]:
            if "," in loc_variant:
                ...
                if city_part in self.STORED_LOCATIONS:
                    ...
                    self.logger.info(
                        f"[OK] Resolved from hardcoded locations (comma split): '{location_name}' -> '{city_part}' -> {bbox}"
                    )
                    self.cache.set(location_name, location_type, bbox)
                    return bbox

        # Try progressive space-split for "City State/Country" patterns without commas
        # e.g., "kathmandu nepal" -> try "kathmandu nepal" (already tried above),
        #        then "kathmandu" alone, or for "Mount Rainier Washington" try
        #        "mount rainier washington", "mount rainier", "mount"
        for loc_variant in [location_lower, location_normalized]:
            words = loc_variant.split()
            if len(words) >= 2:
                ...
                for n in range(len(words) - 1, 0, -1):
                    ...
                    if prefix in self.STORED_LOCATIONS:
                        ...
                        self.logger.info(
                            f"[OK] Resolved from hardcoded locations (space split): '{location_name}' -> '{prefix}' -> {bbox}"
                        )
                        self.cache.set(location_name, location_type, bbox)
                        return bbox

        # Step 1: Semantic preprocessing to improve API query accuracy
        processed_queries = self._preprocess_location_query(
        ...
        all_queries = processed_queries + [
            location_name
        ]  # Try processed queries first, then original

        for query in all_queries:
            # Strategy: International-focused Nominatim with smart queries
            bbox = await self._strategy_international_nominatim(
                location_name, location_type
            )
            if bbox:
                ...
                self.logger.info(
                    f"[GLOBE] Resolved via International Nominatim: {location_name}"
                )
                self.cache.set(location_name, location_type, bbox)
                return bbox

            # Strategy: Multi-language Wikipedia/GeoNames approach
            bbox = await self._strategy_geonames_alternative(
                location_name, location_type
            )
            if bbox:
                # Expand bbox for large geographic features
                bbox = self._expand_bbox_for_large_features(bbox, location_name)
                self.logger.info(
                    f"[DOCS] Resolved via GeoNames alternative: {location_name}"
                )
                self.cache.set(location_name, location_type, bbox)
                return bbox
    ...
    def _looks_like_admin_division(self, location_name: str) -> bool:
        ...
        return (
            name_lower in us_states or "state" in name_lower or "province" in name_lower
        )
    ...
    def _looks_like_city(self, location_name: str) -> bool:
        ...
        return word_count <= 3 and not any(
            term in name_lower
            for term in [
                "mountain",
                "range",
    ...
    def _expand_bbox_for_large_features(
        self, bbox: List[float], location_name: str
    ) -> List[float]:
        """
        ...
        is_large_feature = any(
            keyword in name_lower for keyword in large_feature_keywords
        )
        ...
        if is_large_feature:
            ...
            if current_extent < 0.5:  # Very small bbox (< ~35 miles)
                ...
                self.logger.info(
                    f"[SEARCH] Expanding small bbox for large feature '{location_name}' by {expansion_factor}x"
                )
            elif current_extent < 1.0:  # Small bbox (< ~69 miles)
                expansion_factor = 4.0
                self.logger.info(
                    f"[SEARCH] Expanding bbox for large feature '{location_name}' by {expansion_factor}x"
                )
            ...
            self.logger.info(
                f"[RULER] Expanded bbox from {bbox} to {expanded_bbox} (extent: {current_extent:.2f}° -> {max(new_width, new_height):.2f}°)"
            )
    ...
    async def _strategy_improved_nominatim(
    ...
    def _is_valid_geographic_result(self, result: Dict) -> bool:
        ...
        return any(
            geo_type in place_type or geo_type in osm_class for geo_type in prefer_types
        )
    ...
    def _preprocess_location_query(
        ...
        region_mappings = {
            ...
            "appalachian mountains": [
                ...
                "Appalachian Range Eastern USA",
            ],
            "rocky mountains": [
                "Rocky Mountains USA",
                "Rocky Mountain Range Western USA",
            ],
        ...
        if name_lower in us_landmark_map:
            ...
            if likely_international:
                ...
                if location_type == "city" or self._looks_like_city(location_name):
                    ...
                    processed_queries.extend(
                        [f"{location_name} region", f"{location_name} area"]
                    )
            else:
                ...
                if location_type == "city" or self._looks_like_city(location_name):
                    ...
                    "province",
                    "region",
                ] or self._looks_like_admin_division(location_name):
                    processed_queries.extend(
                        [
                            f"{location_name} state USA",
                    ...
                    processed_queries.extend(
                        [f"{location_name} USA", f"{location_name} United States"]
                    )
    ...
    def _likely_international_location(self, location_name: str) -> bool:
        ...
        return (
            name_lower in international_cities
            or name_lower in international_landmarks
            or name_lower in international_regions
        )

    async def _strategy_international_nominatim(
        self, location_name: str, location_type: str
    ) -> Optional[List[float]]:
    ...
    def _rank_nominatim_international(
    ...
    def _validate_international_result(
        self, location_name: str, bbox: List[float]
    ) -> bool:
        ...
        if name_lower in expected_regions:
            ...
            if not (
                min_lon <= center_lon <= max_lon and min_lat <= center_lat <= max_lat
            ):
                self.logger.info(
                    f"[NO] Rejected {location_name} result ({center_lon:.2f}, {center_lat:.2f}) - outside expected region"
                )
                return False

        return True

    async def _strategy_geonames_alternative(
        self, location_name: str, location_type: str
    ) -> Optional[List[float]]:
    ...
    def _rank_results_by_relevance(
        self, results: List[Dict], location_name: str
    ) -> List[Dict]:
