...
class RenderingConfig:
    ...
    def _get_default_params_to_remove(self) -> List[str]:
        ...
        if self.data_type in [DataType.OPTICAL, DataType.OPTICAL_REFLECTANCE]:
            ...
            # SAR imagery issues
            return always_remove + ["nodata"]  # Can cause issues with dB values
...
EXPLICIT_RENDER_CONFIGS: Dict[str, RenderingConfig] = {
    ...
    "hls2-s30": RenderingConfig(
        ...
        rescale=(
            0,
            6000,
        ),  # Wider range to handle snow (reflectance 8000-9500) without clipping
    ...
    "sentinel-2-l2a": RenderingConfig(
        ...
        rescale=(
            0,
            3000,
        ),  # Sentinel-2 L2A surface reflectance scaled like HLS for proper contrast
    ...
    "landsat-c2-l1": RenderingConfig(
        ...
        rescale=(
            0,
            3000,
        ),  # TOA reflectance 0-10000 range, scaled to 0-3000 for proper contrast
        resampling="bilinear",
    ...
    "landsat-c2-l2": RenderingConfig(
        ...
        rescale=(
            0,
            3000,
        ),  # Surface reflectance 0-10000 range, scaled to 0-3000 for proper contrast (like Sentinel-2 L2A)
    ...
    "modis-09A1-061": RenderingConfig(
        ...
        rescale=(
            0,
            8000,
        ),  # Typical MODIS surface reflectance range for Earth surfaces (scale factor 0.0001)
        resampling="bilinear",
...
def match_collection_family(collection_id: str) -> Optional[DataType]:
    ...
    # Thermal patterns
    if (
        any(pattern in coll_lower for pattern in ["lst", "thermal", "temperature"])
        and "modis" in coll_lower
    ):
    ...
    if any(
        pattern in coll_lower
        for pattern in ["sentinel-2", "landsat", "hls", "modis-09", "naip"]
    ):
...
class HybridRenderingSystem:
    ...
    def get_render_config(
        collection_id: str, query_context: Optional[str] = None
        ...
        """
        logger.info(
            f"[ART] STEP 7 RENDERER: Getting render config for: {collection_id}"
        )
        ...
        if collection_id in EXPLICIT_RENDER_CONFIGS:
            ...
            logger.info(
                "   [OK] CONFIG SOURCE: EXPLICIT_RENDER_CONFIGS (hybrid_rendering_system.py)"
            )
            logger.info("   [DIR] Location: hybrid_rendering_system.py line ~260")
            logger.info(f"   [ART] Assets: {config.assets}")
            logger.info(f"   [CHART] Rescale: {config.rescale}")
            logger.info(
                f"   [PAINT] Color Formula: {config.color_formula[:50] if config.color_formula else 'None'}..."
            )
            logger.info(
                "   [NOTE] Why explicit: Optimized raw bands (B04,B03,B02) instead of 'visual' asset"
            )
            return config
        ...
        if pc_config:
            ...
            logger.info(
                "   [OK] CONFIG SOURCE: PC Repository (pc_rendering_config.json)"
            )
            logger.info(
                f"   [DIR] Location: pc_rendering_config.json > collections.{collection_id}"
            )
            ...
            if pc_config.assets and "visual" in pc_config.assets:
                logger.warning(
                    "   [WARN] PC config uses 'visual' asset which may cause 404s - consider adding to EXPLICIT_RENDER_CONFIGS"
                )
            return pc_config
        ...
        logger.warning(
            "   [DIR] Not in: EXPLICIT_RENDER_CONFIGS (hybrid_rendering_system.py)"
        )
        logger.warning("   [DIR] Not in: pc_rendering_config.json")
        return None
    ...
    def build_titiler_url_params(
        ...
        if not config:
            logger.warning(
                f"No config found for {collection_id}, returning empty params"
            )
            return ""
    ...
    def clean_stac_tilejson_url(url: str, collection_id: str) -> str:
        ...
        if not config:
            logger.warning(
                f"No config found for {collection_id}, returning original URL"
            )
            return url
    ...
    def build_titiler_tilejson_url(item_id: str, collection_id: str) -> str:
        ...
        params = [f"collection={collection_id}", f"item={item_id}"]
...
def _get_cached_mosaic_search_id(cache_key: str) -> Optional[str]:
    ...
    if cache_key in _mosaic_cache:
        ...
        if time.time() - cached["timestamp"] < _MOSAIC_CACHE_TTL:
            logger.info(
                f"[FAST] Mosaic cache HIT: {cache_key[:8]}... -> {cached['search_id'][:16]}..."
            )
...
def _cache_mosaic_search_id(cache_key: str, search_id: str) -> None:
    ...
    _mosaic_cache[cache_key] = {"search_id": search_id, "timestamp": time.time()}
...
async def register_mosaic_search(
    ...
    try:
        # Check cache first
        cache_key = _get_mosaic_cache_key(
            collections, bbox, datetime_range, query_filters
        )
        ...
        search_body["sortby"] = [{"field": "eo:cloud_cover", "direction": "asc"}]
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.info(
                    f"[LAUNCH] Mosaic registration via httpx: {resp.status_code}"
                )
        except ImportError:
            ...
            logger.info(
                f"[SLOW] Mosaic registration via urllib fallback: {response['status_code']}"
            )
...
def build_mosaic_tilejson_url(
    ...
    is_ndvi = any(
        t in query_lower
        for t in ["ndvi", "vegetation index", "normalized difference vegetation"]
    )
    is_ndwi = any(
        t in query_lower for t in ["ndwi", "water index", "normalized difference water"]
    )
    is_false_color = any(
        t in query_lower for t in ["false color", "false-color", "infrared", "nir"]
    )
    ...
    if config:
        ...
        if config.assets and "visual" in config.assets:
            ...
            logger.info(
                f"[SYNC] Mosaic: Config uses 'visual' asset - switching to raw bands for {collection_id}"
            )
    ...
    if needs_raw_bands or not config:
        ...
        if "sentinel-2" in collection_lower:
            if is_ndvi:
                ...
                logger.info(
                    "[ART] Using NDVI expression (B08,B04) for Sentinel-2 mosaic"
                )
            elif is_ndwi:
                ...
                logger.info(
                    "[ART] Using NDWI expression (B03,B08) for Sentinel-2 mosaic"
                )
            elif is_false_color:
                params.extend(["assets=B08", "assets=B04", "assets=B03"])
                params.append("rescale=0,3000")
                logger.info(
                    "[ART] Using false-color NIR bands (B08,B04,B03) for Sentinel-2 mosaic"
                )
            else:
                ...
                logger.info(
                    "[ART] Using raw RGB bands (B04,B03,B02) for Sentinel-2 mosaic"
                )
            ...
            if is_ndvi:
                ...
                params.append("colormap_name=rdylgn")
                logger.info(
                    "[ART] Using NDVI expression (nir08,red) for Landsat mosaic"
                )
            elif is_ndwi:
                ...
                params.append("colormap_name=blues")
                logger.info(
                    "[ART] Using NDWI expression (green,nir08) for Landsat mosaic"
                )
            elif is_false_color:
                params.extend(["assets=nir08", "assets=red", "assets=green"])
                params.append("rescale=0,30000")
                logger.info("[ART] Using false-color NIR bands for Landsat mosaic")
            else:
                ...
                logger.info("[ART] Using raw RGB bands for Landsat mosaic")
            ...
            logger.info("[ART] Using raw RGB bands for HLS mosaic")
        elif "naip" in collection_lower:
            # NAIP RGB
            params.append("assets=image")
            logger.info("[ART] Using image asset for NAIP mosaic")
        else:
            ...
            if render_params:
                ...
                logger.warning(
                    f"[WARN] Unknown collection {collection_id}, using 'visual' asset fallback"
                )
    else:
...
async def get_mosaic_tilejson_url(
    ...
    if not search_id:
        logger.warning(
            "[WARN] Mosaic registration failed, falling back to individual tiles"
        )
        return None
    ...
    tilejson_url = build_mosaic_tilejson_url(
        search_id, collection_id, query_context=query_context
    )
