...
def get_elevation_analysis(
    ...
    try:
        ...
        logger.info(
            f"[TOOL] get_elevation_analysis at ({latitude:.4f}, {longitude:.4f}), radius={radius_km}km"
        )
        ...
        with rasterio.open(dem_url) as src:
            ...
            logger.info(
                f"[TOOL] Elevation: {elev_min:.0f}m - {elev_max:.0f}m, type: {terrain_type}"
            )
        ...
        with rasterio.open(item.assets["data"].href) as src:
            ...
            result = {
                ...
                "traversability": (
                    "easy"
                    if slope_mean < 5
                    else "moderate" if slope_mean < 15 else "difficult"
                ),
            }

            logger.info(
                f"[TOOL] Slope: mean {slope_mean:.1f} deg, {flat_pct:.0f}% flat"
            )
            return json.dumps(result)
    ...
    try:
        ...
        with rasterio.open(item.assets["data"].href) as src:
            ...
            if flat_pct >= 60.0:
                ...
                sloped_total = total - int(
                    flat_mask.sum()
                )  # approximate: flat_mask is per-pixel
                if sloped_total > 0:
                    favorable = (
                        directions.get("S", 0)
                        + directions.get("SE", 0)
                    ...
                    if fav_ratio > 0.50:
                        sun_exposure = "good"
                        sun_note = (
                            "Majority of slopes face south/SE/SW — favorable for solar"
                        )
                    elif fav_ratio > 0.30:
                        ...
                        sun_note = (
                            "Majority of slopes face north — reduced solar exposure"
                        )
                    else:
                        sun_exposure = "moderate"
            ...
            logger.info(
                f"[TOOL] Aspect: dominant {dominant}, flat {flat_pct:.0f}%, sun exposure {sun_exposure}"
            )
            return json.dumps(result)
...
def find_flat_areas(
    ...
    try:
        ...
        logger.info(
            f"[TOOL] find_flat_areas at ({latitude:.4f}, {longitude:.4f}), max_slope={max_slope_degrees} deg"
        )
        ...
        with rasterio.open(item.assets["data"].href) as src:
            ...
            suitable = (
                "excellent"
                if flat_pct > 50
                else "good" if flat_pct > 20 else "limited" if flat_pct > 5 else "poor"
            )
            ...
            logger.info(
                f"[TOOL] Flat areas: {flat_pct:.1f}% below {max_slope_degrees} deg"
            )
            return json.dumps(result)
...
def analyze_flood_risk(
    ...
    try:
        ...
        if not items:
            return json.dumps(
                {
                    "error": "No JRC Global Surface Water data available",
                    "flood_risk": "unknown",
                }
            )
        ...
        if "occurrence" not in item.assets:
            return json.dumps(
                {"error": "No occurrence data in JRC-GSW item", "flood_risk": "unknown"}
            )
        ...
        with rasterio.open(occurrence_url) as src:
            ...
            if window.width < 1 or window.height < 1:
                return json.dumps(
                    {"error": "Area too small for analysis", "flood_risk": "unknown"}
                )
            ...
            if not np.any(valid_mask):
                return json.dumps(
                    {"error": "No valid water occurrence data", "flood_risk": "unknown"}
                )
            ...
            pct_frequently_flooded = float(
                np.sum(valid_data > 25) / len(valid_data) * 100
            )
            ...
            logger.info(
                f"[TOOL] Flood risk: {risk_level} (max occurrence: {max_occurrence:.0f}%)"
            )
            return json.dumps(result)
...
def analyze_water_proximity(
    latitude: float,
    longitude: float,
    radius_km: float = 5.0,
    required_setback_meters: float = 500.0,
) -> str:
    ...
    try:
        ...
        logger.info(
            f"[TOOL] analyze_water_proximity at ({latitude:.4f}, {longitude:.4f})"
        )
        ...
        with rasterio.open(occurrence_url) as src:
            ...
            logger.info(
                f"[TOOL] Water proximity: {center_distance_meters:.0f}m, setback {'OK' if setback_satisfied else 'FAILED'}"
            )
            return json.dumps(result)
...
def analyze_environmental_sensitivity(
    ...
    try:
        ...
        logger.info(
            f"[TOOL] analyze_environmental_sensitivity at ({latitude:.4f}, {longitude:.4f})"
        )
        ...
        with rasterio.open(map_url) as src:
            ...
            class_counts = {
                ...
                "herbaceous_wetland": float(
                    np.sum(landcover == 90) / total_pixels * 100
                ),
            ...
            if class_counts["herbaceous_wetland"] > 5:
                constraints.append(
                    f"Wetlands ({class_counts['herbaceous_wetland']:.1f}%) - may require wetland mitigation"
                )
            if class_counts["mangroves"] > 1:
                constraints.append(
                    f"Mangroves ({class_counts['mangroves']:.1f}%) - protected habitat, development restricted"
                )
            if class_counts["tree_cover"] > 30:
                constraints.append(
                    f"Forest ({class_counts['tree_cover']:.1f}%) - may require deforestation permit"
                )
            if class_counts["permanent_water"] > 10:
                constraints.append(
                    f"Water bodies ({class_counts['permanent_water']:.1f}%) - setback requirements apply"
                )
            ...
            result = {
                "land_cover_breakdown_percent": {
                    k: round(v, 1) for k, v in class_counts.items() if v > 0.5
                },
                ...
                "environmental_constraints": (
                    constraints
                    if constraints
                    else ["No major environmental constraints identified"]
                ),
                "data_source": "ESA WorldCover 2021 (10m resolution)",
            }

            logger.info(
                f"[TOOL] Environmental sensitivity: {sensitivity_level} ({sensitive_percent:.0f}% sensitive)"
            )
            return json.dumps(result)
