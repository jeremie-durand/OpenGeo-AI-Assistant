...
class GeospatialTools:
    ...
    async def calculate_terrain_slope(
        ...
        try:
            logger.info(
                f" Calculating terrain slope for ({latitude:.6f}, {longitude:.6f}), radius={radius_miles}mi"
            )
            ...
            elevation_data = await self._download_raster(
                collection_id="cop-dem-glo-30", bbox=bbox, asset_key="data"
            )
            ...
            steep_pct = float(
                np.sum(valid_slopes > self.STEEP_SLOPE_THRESHOLD)
                / len(valid_slopes)
                * 100
            )
            moderate_pct = float(
                np.sum(
                    (valid_slopes >= self.MODERATE_SLOPE_THRESHOLD)
                    & (valid_slopes <= self.STEEP_SLOPE_THRESHOLD)
                )
                / len(valid_slopes)
                * 100
            )
            gentle_pct = float(
                np.sum(valid_slopes < self.MODERATE_SLOPE_THRESHOLD)
                / len(valid_slopes)
                * 100
            )

            logger.info(
                f" Slope analysis: avg={avg_slope:.1f}°, max={max_slope:.1f}°, steep={steep_pct:.1f}%"
            )
    ...
    async def measure_elevation_profile(
        ...
        try:
            logger.info(
                f" Measuring elevation profile for ({latitude:.6f}, {longitude:.6f})"
            )
            ...
            logger.info(
                f" Elevation: min={min_elev:.0f}m, max={max_elev:.0f}m, avg={avg_elev:.0f}m, relief={relief:.0f}m"
            )
    ...
    async def detect_water_bodies(
        ...
        try:
            logger.info(
                f" Detecting water bodies for ({latitude:.6f}, {longitude:.6f})"
            )
            ...
            sar_data = await self._download_raster(
                collection_id="sentinel-1-grd", bbox=bbox, asset_key="vv", days_back=30
            )
    ...
    async def calculate_vegetation_index(
        ...
        try:
            logger.info(
                f" Calculating {index_type} for ({latitude:.6f}, {longitude:.6f})"
            )
            ...
            if index_type == "NDVI":
                ...
                nir_data = await self._download_raster(
                    "sentinel-2-l2a", bbox, "B08", days_back=30
                )
                red_data = await self._download_raster(
                    "sentinel-2-l2a", bbox, "B04", days_back=30
                )
                ...
                green_data = await self._download_raster(
                    "sentinel-2-l2a", bbox, "B03", days_back=30
                )
                nir_data = await self._download_raster(
                    "sentinel-2-l2a", bbox, "B08", days_back=30
                )
                ...
                return {
                    "error": f"Index type {index_type} not yet implemented",
                    "confidence": "low",
                }

            # Remove invalid values
            valid_index = index_data[
                (~np.isnan(index_data)) & (index_data >= -1) & (index_data <= 1)
            ]
            ...
            if index_type == "NDVI":
                ...
                sparse_pct = float(
                    np.sum((valid_index >= 0.2) & (valid_index <= 0.6))
                    / len(valid_index)
                    * 100
                )
    ...
    async def detect_active_fires(
        ...
        try:
            logger.info(
                f" Detecting active fires for ({latitude:.6f}, {longitude:.6f})"
            )
            ...
            logger.info(
                f" Fire detection: high={high_conf}, nominal={nominal}, low={low_conf}"
            )
    ...
    async def classify_land_cover(
        ...
        try:
            logger.info(
                f" Classifying land cover for ({latitude:.6f}, {longitude:.6f})"
            )

            # Get NDVI and NDWI
            ndvi_result = await self.calculate_vegetation_index(
                latitude, longitude, radius_miles, "NDVI"
            )
            ndwi_result = await self.calculate_vegetation_index(
                latitude, longitude, radius_miles, "NDWI"
            )
            ...
            water_pct = max(
                0, ndwi_result.get("avg_index", 0) * 30
            )  # Rough approximation
            ...
            urban_pct = max(
                0, 100 - water_pct - dense_veg_pct - sparse_veg_pct - bare_soil_pct
            )

            logger.info(
                f" Land cover: water={water_pct:.1f}%, veg={dense_veg_pct+sparse_veg_pct:.1f}%"
            )
    ...
    async def _download_raster(
        ...
        try:
            ...
            search_params = {"collections": [collection_id], "bbox": bbox, "limit": 1}
            ...
            if asset_key not in item.assets:
                ...
                logger.warning(
                    f" Asset {asset_key} not found. Available: {available_assets}"
                )
                if available_assets:
