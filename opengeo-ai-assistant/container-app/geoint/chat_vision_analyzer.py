...
class ChatVisionAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(
            f" ChatVisionAnalyzer initialized with model: {self.deployment_name}"
        )
        ...
        if conversation_history and len(conversation_history) > 0:
            ...
            if any(
                ...
                if any(pattern in query_lower for pattern in followup_patterns):
                    logger.info(
                        f" Contextual vision query detected (follow-up): '{query}'"
                    )
                    return True
    ...
    async def analyze_visible_imagery(
        ...
        try:
            ...
            logger.info(
                f" Map center: ({map_bounds.get('center_lat')}, {map_bounds.get('center_lng')})"
            )
    ...
    async def _fetch_visible_imagery(
        ...
        """
        try:
            metadata = {"source": collection_id or "Unknown", "bounds": map_bounds}
            ...
            if imagery_url:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        imagery_url, timeout=aiohttp.ClientTimeout(total=30)
                    ) as response:
                        if response.status == 200:
                            image_bytes = await response.read()
                            metadata["url"] = imagery_url
                            logger.info(
                                f" Fetched {len(image_bytes)} bytes from provided URL"
                            )
                            return image_bytes, metadata
                        else:
                            logger.warning(
                                f" Failed to fetch from provided URL: {response.status}"
                            )
            ...
            if map_bounds and collection_id:
                ...
                if static_map_url:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            static_map_url, timeout=aiohttp.ClientTimeout(total=30)
                        ) as response:
                            if response.status == 200:
                                image_bytes = await response.read()
                                ...
                                logger.info(
                                    f" Fetched {len(image_bytes)} bytes from TiTiler static map"
                                )
                                return image_bytes, metadata
            ...
            if center_lat and center_lng:
                ...
                image_bytes, sentinel_metadata = (
                    await vision_analyzer._fetch_satellite_image(
    ...
    def _build_static_map_url(
        self, map_bounds: Dict[str, float], collection_id: str
    ) -> Optional[str]:
        """
...
If they ask "what is in this image?" give a comprehensive overview. If they ask specific questions, focus your answer on what they're asking about."""
            ...
            user_prompt_parts.append(
                f"- Location: {map_bounds.get('center_lat', 'N/A')}°N, {map_bounds.get('center_lng', 'N/A')}°E"
            )
            ...
            logger.info(
                f" GPT-5 conversational analysis completed ({len(analysis_text)} characters)"
            )
