...
async def terrain_analysis_agent(
    ...
    - Visual analysis (GPT-5 Vision on satellite imagery)
    ...
    try:
        ...
        response = {
            ...
            "imagery_metadata": {
                "source": "Copernicus DEM + GPT-5 Vision",
        ...
        if result.get("tool_calls"):
            response["features"] = [
                f"DEM analysis via {tool}" for tool in result["tool_calls"]
            ]

        logger.info(
            f" Terrain analysis completed with {len(result.get('tool_calls', []))} tool calls"
        )
        return response
...
async def mobility_analysis_agent(
    ...
    try:
        if latitude_b is not None and longitude_b is not None:
            logger.info(
                f" Mobility analysis agent: A({latitude}, {longitude}) -> B({latitude_b}, {longitude_b})"
            )
        else:
            logger.info(
                f" Mobility analysis agent called for ({latitude}, {longitude})"
            )
        ...
        logger.info(" Mobility analysis completed")
        return result
...
async def extreme_weather_analysis_agent(
    ...
    try:
        ...
        if result.get("tool_calls"):
            response["features"] = [
                f"Climate analysis via {tc.get('tool', tc) if isinstance(tc, dict) else tc}"
                for tc in result["tool_calls"]
            ]

        logger.info(
            f" Extreme weather analysis completed with {len(result.get('tool_calls', []))} tool calls"
        )
        return response
...
async def _download_and_visualize_raster(
    latitude: float, longitude: float, date: str, collection_id: str, bbox: List[float]
) -> Optional[str]:
    """
    ...
    try:
        ...
        catalog = pystac_client.Client.open(
            os.getenv("STAC_API_URL", "http://localhost:8081")
        )
        ...
        if data.ndim == 2:
            ...
            if "fire" in collection_id.lower() or "thermal" in collection_id.lower():
                ...
                "ndvi" in collection_id.lower() or "vegetation" in collection_id.lower()
            ):
                cmap = "RdYlGn"
                label = "NDVI"
            else:
...
async def comparison_analysis_agent(
    ...
    try:
        ...
        logger.info(
            f" Before screenshot: {'Provided' if before_screenshot_base64 else 'None'} ({len(before_screenshot_base64) if before_screenshot_base64 else 0} chars)"
        )
        logger.info(
            f" After screenshot: {'Provided' if after_screenshot_base64 else 'None'} ({len(after_screenshot_base64) if after_screenshot_base64 else 0} chars)"
        )
        logger.info("")

        # Validate at least one data source
        has_screenshots = bool(before_screenshot_base64 and after_screenshot_base64)
        can_download_rasters = bool(download_rasters and collection_id)
        ...
        if not has_screenshots and not can_download_rasters:
            ...
            raise ValueError(
                "Either screenshots (both before+after) OR raster download capability (collection_id + download_rasters=true) must be provided"
            )
        ...
        if download_rasters and collection_id:
            ...
            if before_raster_base64:
                logger.info(
                    f" BEFORE raster downloaded successfully ({len(before_raster_base64)} chars)"
                )
            else:
            ...
            if after_raster_base64:
                logger.info(
                    f" AFTER raster downloaded successfully ({len(after_raster_base64)} chars)"
                )
            else:
            ...
            if before_raster_base64 and after_raster_base64:
                ...
                logger.warning(
                    " Raster download incomplete, proceeding with screenshots only"
                )
        else:
            logger.info(
                "[SKIP] Raster download skipped (download_rasters=False or no collection_id)"
            )

        logger.info("")

        # Final data source check
        has_rasters = bool(before_raster_base64 and after_raster_base64)
        ...
        logger.info(
            f"   - Total images for GPT-5: {sum([has_screenshots * 2, has_rasters * 2])}"
        )

        if not has_screenshots and not has_rasters:
            logger.error(" No data sources available after download attempts")
            raise ValueError(
                "No valid data sources (screenshots or rasters) available for comparison analysis"
            )
        ...
        if collection_id:
            if "modis" in collection_id.lower() or "fire" in collection_id.lower():
                ...
                "landsat" in collection_id.lower()
                or "sentinel" in collection_id.lower()
            ):
...
You will analyze visual map screenshots and/or raw raster data to provide comprehensive insights."""
        ...
        if before_screenshot_base64:
            image_description += (
                f"{image_count}. BEFORE map screenshot (visual representation)\n"
            )
            image_count += 1
        ...
        if after_screenshot_base64:
            image_description += (
                f"{image_count}. AFTER map screenshot (visual representation)\n"
            )
            image_count += 1
...
{'- Identify areas with highest/lowest values and changes' if before_raster_base64 and after_raster_base64 else '- Identify visual anomalies and features'}
{'- Correlate raster intensity with visual map features' if before_raster_base64 and after_raster_base64 else '- Describe color and texture changes'}
...
"""
        ...
        if before_screenshot_base64:
            ...
            image_count += 1
            logger.info(
                f"   {image_count}. BEFORE map screenshot ({len(before_screenshot_base64)} chars)"
            )
        ...
        if before_raster_base64:
            ...
            image_count += 1
            logger.info(
                f"   {image_count}. BEFORE raster data - {raster_type} ({len(before_raster_base64)} chars)"
            )
        ...
        if after_screenshot_base64:
            ...
            image_count += 1
            logger.info(
                f"   {image_count}. AFTER map screenshot ({len(after_screenshot_base64)} chars)"
            )
        ...
        if after_raster_base64:
            ...
            image_count += 1
            logger.info(
                f"   {image_count}. AFTER raster data - {raster_type} ({len(after_raster_base64)} chars)"
            )
        ...
        logger.info(
            f"   - Image count: {len([c for c in user_content if c['type'] == 'image_url'])}"
        )
        logger.info(f"   - Max tokens: {payload['max_completion_tokens']}")
        ...
        analysis_text = re.sub(
            r"^(\d+\.\s+[^\n]+)", r"**\1**", analysis_text, flags=re.MULTILINE
        )
        ...
        if before_raster_base64 and after_raster_base64:
            logger.info(
                f" Analysis included quantitative raster data from {collection_id}"
            )
        ...
        logger.info(
            f"[TIME]  Time span: {_calculate_time_span(before_date, after_date)}"
        )
        logger.info(
            f" Confidence: {0.85 if (before_raster_base64 and after_raster_base64) else 0.8}"
        )
        logger.info("")
        ...
        return {
            ...
            "raster_analysis_included": bool(
                before_raster_base64 and after_raster_base64
            ),
            "confidence": (
                0.85 if (before_raster_base64 and after_raster_base64) else 0.8
            ),
...
async def animation_generation_agent(
    ...
    try:
        logger.info(
            f" Animation generation agent called for ({latitude}, {longitude}) [{start_date} -> {end_date}]"
        )
