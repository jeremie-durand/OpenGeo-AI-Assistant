...
STAC_URL = (
    _stac_api_url.rsplit("/search", 1)[0].rstrip("/")
    if _stac_api_url
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if features:
                ...
                tile_urls.append(
                    f"https://planetarycomputer.microsoft.com/api/data/v1/item/tilejson.json?collection={stac_collection}&item={item_id}&assets={asset}{extra}"
                )
            return {
                "features": features,
                "tile_urls": tile_urls,
...
def _parse_coordinates_to_bbox(
    location: str, buffer_deg: float = 0.15
) -> Optional[List[float]]:
...
def _resolve_location_sync(location: str) -> Optional[List[float]]:
    ...
    if coord_bbox:
        logger.info(
            f"Parsed coordinates directly from '{location}' -> bbox={coord_bbox}"
        )
        return coord_bbox
...
def _fetch_tile_preview_sync(
    ...
    try:
        ...
        tile_url = (
            tiles_template.replace("{z}", str(z))
            .replace("{x}", str(x))
            .replace("{y}", str(y))
        )
...
def compare_temporal_imagery(
    ...
    try:
        ...
        if not before_date or not after_date:
            logger.error(
                f"[compare_temporal_imagery] Date parse failed — before='{before_period}' -> {before_date!r}, after='{after_period}' -> {after_date!r}"
            )
            result = {
                "status": "error",
                "message": f"Could not parse date ranges. Before: '{before_period}', After: '{after_period}'. Use MM/YYYY or Month YYYY format.",
            }
        ...
        if not bbox:
            result = {
                "status": "error",
                "message": f"Could not resolve location: '{location}'. Please provide a valid city or region name.",
            }
            return json.dumps(result)

        collection = COLLECTION_MAP.get(analysis_type.lower(), "sentinel-2-l2a")

        before_result = _execute_stac_search_sync(
            collection, bbox, before_date, limit=3
        )
        ...
        if not before_features and not after_features:
            result = {
                "status": "error",
                "message": f"No imagery found for {location} in either time period.",
            }
            return json.dumps(result)
        ...
        result = {
            ...
            "before": {
                ...
                "best_scene_date": (
                    _get_scene_date(before_features[0]) if before_features else None
                ),
            },
            "after": {
                "datetime": after_date,
                ...
                "best_scene_date": (
                    _get_scene_date(after_features[0]) if after_features else None
                ),
            },
        ...
        return json.dumps(
            {"status": "error", "message": f"Comparison failed: {str(e)}"}
        )
    ...
    try:
        ...
        if not dt_range:
            return json.dumps(
                {
                    "status": "error",
                    "message": f"Could not parse time period: '{time_period}'",
                }
            )
...
def analyze_comparison_imagery(
    location: str,
    before_tile_url: str,
    after_tile_url: str,
    ...
    try:
        ...
        before_image = _fetch_tile_preview_sync(
            before_tile_url, latitude=lat, longitude=lon
        )
        after_image = _fetch_tile_preview_sync(
            after_tile_url, latitude=lat, longitude=lon
        )
