...
class RasterDataFetcher:
    ...
    async def fetch_terrain_data(
        ...
        try:
            ...
            result = {
                ...
                "terrain_classification": dem_result.get(
                    "terrain_classification", "unknown"
                ),
                "spectral_indices": spectral_result.get("indices", {}),
                "vegetation_classification": spectral_result.get(
            ...
            logger.info(
                f" Terrain data fetched: elevation range {result['elevation_stats'].get('min', 0):.0f}-{result['elevation_stats'].get('max', 0):.0f}m"
            )
            return result
    ...
    async def _fetch_dem_data(
        self, bbox: List[float], latitude: float, longitude: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            search = self.catalog.search(
                collections=[DEM_COLLECTION], bbox=bbox, limit=1
            )
            ...
            with rasterio.open(dem_url) as src:
                ...
                slope, aspect = self._calculate_slope_aspect(
                    elevation, src.res[0]  # pixel resolution in meters
                )
    ...
    def _calculate_slope_aspect(
        self, elevation: np.ndarray, cell_size: float
    ...
    def _classify_terrain(
        self, elevation_stats: Dict[str, float], slope_stats: Dict[str, float]
    ) -> str:
    ...
    async def _fetch_spectral_data(
        self, bbox: List[float], latitude: float, longitude: float, collection_id: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _calculate_ndvi(
        self, red_url: str, nir_url: str, bbox: List[float]
    ...
    async def _calculate_naip_ndvi(
        self, image_url: str, bbox: List[float]
    ) -> Optional[Dict[str, float]]:
    ...
    def format_terrain_summary(self, data: Dict[str, Any]) -> str:
        """
        Format terrain data into a human-readable summary for GPT-5 context.
        """
        ...
        if elev:
            lines.append(
                f"- **Elevation:** {elev.get('min', 0):.0f}m - {elev.get('max', 0):.0f}m (mean: {elev.get('mean', 0):.0f}m, range: {elev.get('range', 0):.0f}m)"
            )
        ...
        if slope:
            lines.append(
                f"- **Slope:** avg {slope.get('mean', 0):.1f}° (max: {slope.get('max', 0):.1f}°)"
            )
        ...
        if aspect.get("dominant_direction"):
            lines.append(
                f"- **Dominant Slope Direction:** {aspect.get('dominant_direction')}"
            )
        ...
        if terrain_class:
            lines.append(
                f"- **Terrain Type:** {terrain_class.replace('_', ' ').title()}"
            )
        ...
        if ndvi:
            lines.append(
                f"- **NDVI:** {ndvi.get('mean', 0):.2f} (range: {ndvi.get('min', 0):.2f} to {ndvi.get('max', 0):.2f})"
            )
