  ...
  generateReport() {
    const report = {
      ...
      summary: {
        ...
        successfulTiles: Array.from(this.tileRequests.values()).filter(
          (r) => r.status === 'success'
        ).length,
        failedTiles: Array.from(this.tileRequests.values()).filter((r) => r.status === 'failed')
          .length,
        errorTiles: Array.from(this.tileRequests.values()).filter((r) => r.status === 'error')
          .length,
...
window.STACDebugger = {
  ...
  async testTilejsonAsset(tilejsonUrl, feature, featureIndex) {
    try {
      window.MapDebugger.log(
        'DEBUG',
        'TILEJSON_TEST',
        `Testing tilejson URL for feature ${featureIndex}`,
        {
          url: tilejsonUrl,
        }
      );
      ...
      if (response.ok) {
        ...
        window.MapDebugger.log(
          'INFO',
          'TILEJSON_SUCCESS',
          `Tilejson loaded for feature ${featureIndex}`,
          {
        ...
        window.MapDebugger.log(
          'ERROR',
          'TILEJSON_FAILED',
          `Tilejson failed for feature ${featureIndex}`,
          {
            status: response.status,
            statusText: response.statusText,
          }
      ...
      window.MapDebugger.log(
        'ERROR',
        'TILEJSON_ERROR',
        `Tilejson error for feature ${featureIndex}`,
        {
          error: error.message,
        }
      );
    ...
    for (const zoom of testZooms) {
      ...
      if (result.success) {
        window.MapDebugger.log(
          'INFO',
          'WORKING_TILE_FOUND',
          `[OK] Found working tile for feature ${featureIndex}`,
          {
            zoom,
            x,
            y,
