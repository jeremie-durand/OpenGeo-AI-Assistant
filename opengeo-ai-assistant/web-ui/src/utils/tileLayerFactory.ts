...
export function createTileLayer(options: TileLayerOptions, leaflet: any): any {
  ...
  if (
    collectionLower.includes('sentinel-2') ||
    collectionLower.includes('landsat') ||
    collectionLower.includes('hls') ||
    collectionLower.includes('naip')
  ) {
    ...
    isElevation ||
    collectionLower.includes('dem') ||
    collectionLower.includes('elevation')
  ) {
    opacity = 0.65;
...
export async function createMultipleTileLayers(
  ...
  console.log(
    `[TileLayerFactory] Creating ${tiles.length} Leaflet tile layers for seamless coverage`
  );
  const layers: any[] = [];
  let successCount = 0;
  let errorCount = 0;
  const isElevation =
    collection.toLowerCase().includes('dem') || collection.toLowerCase().includes('elevation');
  for (const tile of tiles) {
    try {
      ...
      successCount++;
      console.log(
        `[TileLayerFactory] Created Leaflet layer ${successCount}/${tiles.length}: ${tile.itemId}`
      );
    } catch (error) {
  ...
  console.log(
    `[TileLayerFactory] Multi-tile Leaflet layer creation complete. Success: ${successCount}, Errors: ${errorCount}`
  );
...
export function validateAndClampBounds(bounds: number[]): number[] | undefined {
  ...
  if (!isValidBound(west) || !isValidBound(south) || !isValidBound(east) || !isValidBound(north)) {
  ...
  if (west >= east || south >= north) {
    console.warn(
      '[TileLayerFactory] Invalid bounds geometry (west >= east or south >= north):',
      bounds
    );
    return undefined;
  }
  ...
  if (clampedSouth >= clampedNorth) {
    console.warn(
      '[TileLayerFactory] Bounds degenerate after WebMercator clamping (south >= north):',
  ...
  if (clampedWest >= clampedEast) {
    console.warn('[TileLayerFactory] Bounds degenerate after clamping (west >= east):', {
      original: bounds,
      clamped: [clampedWest, clampedSouth, clampedEast, clampedNorth],
    });
    return undefined;
  }
...
export function ensureHighResolution(tileUrl: string): string {
  ...
  if (isNativeTiles) {
    console.log(
      '[TileLayerFactory] Native tiles detected - NOT adding tile_scale (would cause 404)'
    );
    return tileUrl;
  }
...
export function isThermalCollection(collection: string): boolean {
  const lower = collection.toLowerCase();
  return lower.includes('thermal') || lower.includes('modis-11') || lower.includes('lst'); // Land Surface Temperature
}
...
export function isFireCollection(collection: string): boolean {
  const lower = collection.toLowerCase();
  return lower.includes('fire') || lower.includes('modis-14');
}
