...
export function prepareMultiTileData(data: SatelliteData): PreparedTileData[] {
  ...
  console.log(
    ` [SatelliteDataHelpers] Preparing ${data.all_tile_urls.length} tiles for multi-tile rendering`
  );
 ...
 */
export function extractAssets(item: STACItem, assetTypes?: string[]): Record<string, STACAsset> {
  if (!item.assets) {
    return {};
  }
...
export function isThermalData(data: SatelliteData): boolean {
  ...
  return (
    collection.includes('thermal') || collection.includes('modis-11') || collection.includes('lst')
  );
}
...
export function isFireData(data: SatelliteData): boolean {
  ...
  return collection.includes('fire') || collection.includes('modis-14');
}
...
export function isSARData(data: SatelliteData): boolean {
  ...
  return collection.includes('sentinel-1') || collection.includes('sar');
}
...
export function applyAssetFixes(tileUrl: string, collection: string): string {
  ...
  if (collection.toLowerCase().includes('sentinel-2-l2a')) {
    if (
      tileUrl.includes('assets=red') ||
      tileUrl.includes('assets=green') ||
      tileUrl.includes('assets=blue')
    ) {
      const fixedUrl = tileUrl.replace(/assets=(red|green|blue|red,green,blue)/g, 'assets=visual');

      if (fixedUrl !== tileUrl) {
        console.log(
          ' [SatelliteDataHelpers] Fixed Sentinel-2 L2A assets: [red,green,blue] -> [visual]'
        );
        return fixedUrl;
      }
    }
