...
const DataLegend: React.FC<DataLegendProps> = ({
    ...
    const fetchColormap = async () => {
      try {
        ...
        if (rgbCollections.some((rgb) => collection.toLowerCase().includes(rgb))) {
          console.log(
            `ℹ️ Skipping colormap fetch for RGB collection '${collection}' (not needed for optical imagery)`
          );
        ...
        if (data.success && data.css_gradient) {
          ...
          console.log(
            `Loaded TiTiler colormap '${data.colormap_name}' for collection '${collection}'`
          );
        } else {
  ...
  const getDataGradient = (collection: string) => {
    ...
    if (colormapData && colormapData.css_gradient) {
      ...
      if (
        lowerCollection.includes('dem') ||
        lowerCollection.includes('elevation') ||
        lowerCollection.includes('srtm') ||
        lowerCollection.includes('cop-dem')
      ) {
        return {
          ...
          labels: [
            ...
            {
              position: 25,
              value: `${Math.round(min + (max - min) * 0.25)}m`,
              description: 'Foothills',
            },
            {
              position: 50,
              value: `${Math.round(min + (max - min) * 0.5)}m`,
              description: 'Mountains',
            },
            {
              position: 75,
              value: `${Math.round(min + (max - min) * 0.75)}m`,
              description: 'High Peaks',
            },
      ...
      if (
        lowerCollection.includes('ndvi') ||
        lowerCollection.includes('13q1') ||
        lowerCollection.includes('13a1')
      ) {
        return {
    ...
    if (
      ...
      return {
        ...
        gradientColors:
          'linear-gradient(to top, #2E7D32 0%, #4DB560 15%, #8BC34A 30%, #D4B86A 45%, #C49A6C 60%, #8B6F47 75%, #6D4C3D 88%, #FFFFFF 100%)',
        labels: [
    ...
    if (lowerCollection.includes('sentinel-1') || lowerCollection.includes('sar')) {
      return {
        ...
        gradientColors:
          'linear-gradient(to top, #000000 0%, #333333 25%, #666666 50%, #999999 75%, #FFFFFF 100%)',
        labels: [
    ...
    // Fire/Thermal Data
    if (
      lowerCollection.includes('fire') ||
      lowerCollection.includes('thermal') ||
      lowerCollection.includes('modis-fire') ||
      lowerCollection.includes('goes-glm') ||
      lowerCollection.includes('modis-14a') ||
      lowerCollection.includes('modis-64a')
    ) {
      return {
        title: 'Fire Intensity',
        gradientColors:
          'linear-gradient(to top, #000080 0%, #0000FF 20%, #FFFF00 40%, #FF8000 70%, #FF0000 100%)',
        labels: [
    ...
    if (lowerCollection.includes('15a2h') || lowerCollection.includes('lai')) {
      return {
        ...
        gradientColors:
          'linear-gradient(to top, #440154 0%, #414487 25%, #2a788e 50%, #22a884 75%, #7ad151 100%)',
        labels: [
    ...
    if (lowerCollection.includes('17a2h') || lowerCollection.includes('gpp')) {
      return {
        ...
        gradientColors:
          'linear-gradient(to top, #0d0887 0%, #7e03a8 25%, #cc4778 50%, #f89441 75%, #f0f921 100%)',
        labels: [
    ...
    if (
      lowerCollection.includes('ndvi') ||
      lowerCollection.includes('vegetation') ||
      lowerCollection.includes('modis-ndvi')
    ) {
      return {
        title: 'Vegetation Index',
        gradientColors:
          'linear-gradient(to top, #8B4513 0%, #D2B48C 20%, #FFFF99 40%, #90EE90 70%, #006400 100%)',
        labels: [
    ...
    if (lowerCollection.includes('snow') || lowerCollection.includes('modis-snow')) {
      return {
        title: 'Snow Cover',
        gradientColors:
          'linear-gradient(to top, #8B4513 0%, #32CD32 25%, #87CEEB 50%, #E0E0E0 75%, #FFFFFF 100%)',
        labels: [
    ...
    if (
      lowerCollection.includes('ocean') ||
      lowerCollection.includes('sst') ||
      lowerCollection.includes('sea-surface')
    ) {
      return {
        title: 'Sea Surface Temp',
        gradientColors:
          'linear-gradient(to top, #000080 0%, #0066CC 25%, #00CCCC 50%, #FFFF00 75%, #FF0000 100%)',
        labels: [
    ...
    if (
      lowerCollection.includes('climate') ||
      lowerCollection.includes('precipitation') ||
      lowerCollection.includes('temperature')
    ) {
      return {
        title: 'Climate Data',
        gradientColors:
          'linear-gradient(to top, #800080 0%, #0000FF 25%, #00FFFF 50%, #FFFF00 75%, #FF0000 100%)',
        labels: [
  ...
  return (
    <div
      style={{
        ...
        right: '20px',
        background:
          'linear-gradient(135deg, rgba(255, 255, 255, 0.85) 0%, rgba(248, 250, 252, 0.75) 50%, rgba(241, 245, 249, 0.85) 100%)',
        padding: '12px',
        borderRadius: '8px',
