...
function extractTextFromResponse(content: any): string {
  ...
  console.log(
    ' extractTextFromResponse: Input content preview:',
  ...
  if (typeof content === 'string') {
    ...
    if (
      contentStr.includes('ChatMessageContent(') &&
      contentStr.includes('ChatCompletionMessage(content=')
    ) {
      ...
      const messageContentMatch = contentStr.match(
        /ChatCompletionMessage\(content='([^']*(?:\\'[^']*)*?)'/s
      );
      ...
      if (generalContentMatch && generalContentMatch[1] && generalContentMatch[1].length > 100) {
        ...
        console.log(
          ' extractTextFromResponse:  General content pattern did not match or too short'
        );
      }
      ...
      console.log(
        ' extractTextFromResponse: Not a ChatMessageContent format, treating as regular string'
      );
    }
  ...
  if (typeof content === 'object' && content !== null) {
    ...
    try {
      ...
      console.log(
        ' extractTextFromResponse: Object property access failed, trying string conversion'
      );
    }
...
function renderMessageHTML(content: string): string {
  ...
  s = s.replace(/^\s*Preview:\s*.*$/gim, '(shown on map)');
  ...
  s = s.replace(
    /^(\d+\))\s+(.+)$/gm,
    '<strong style="font-size: 1.1em; display: block; margin-top: 16px; margin-bottom: 8px; color: #1e3a8a;">$1 $2</strong>'
  );

  // Convert markdown headers (##, ###, ####) to styled headers
  s = s.replace(
    /^#{2,4}\s+(.+)$/gm,
    '<strong style="font-size: 1.1em; display: block; margin-top: 16px; margin-bottom: 8px; color: #1e3a8a;">$1</strong>'
  );
...
interface ChatProps {
  ...
  } | null; // Pin A/B coordinates for mobility
  ...
  ) => void; // Update terrain session (e.g. after first response creates session)
...
const Chat: React.FC<ChatProps> = ({
  ...
  const [pendingAnalysisType, setPendingAnalysisType] = useState<'raster' | 'screenshot' | null>(
    null
  );
      ...
      if (query && query.trim()) {
        ...
        requestAnimationFrame(() => {
          setTimeout(() => {
            ...
            console.log(
              ' Chat: Current state (from refs after 50ms delay) - selectedModule:',
              currentSelectedModule,
              'vision_pin:',
              currentMapContext?.vision_pin,
              'vision_mode:',
              currentMapContext?.vision_mode
            );
            ...
            const isVisionActive =
              currentSelectedModule === 'vision' || currentMapContext?.vision_mode === true;

            if (requiresVision && !isVisionActive) {
              console.warn(
                ' Chat: Vision validation failed - selectedModule is:',
                currentSelectedModule,
                'vision_mode:',
                currentMapContext?.vision_mode
              );
              validationErrors.push(
                '**Vision module is not active.** Please click the Vision button in the module panel first.'
              );
            }
            ...
            if (requiresPin && !currentMapContext?.vision_pin) {
              validationErrors.push(
                '**No pin dropped.** After enabling Vision mode, click on the map to drop a pin at the location you want to analyze.'
              );
            }
  ...
  useEffect(() => {
    ...
    if (mobilityAnalysisResult.type === 'info') {
      ...
      setMessages((prev) => {
        ...
        const result =
          mobilityAnalysisResult.data?.result ?? mobilityAnalysisResult.data?.data?.result;
        if (result) {
          let content = '';
    ...
    mutationFn: async (message: string) => {
      try {
        ...
        const hasVisionContext =
          selectedModule === 'vision' && currentMapCtx?.vision_mode && currentMapCtx?.vision_pin;
        ...
        if (terrainSessionNow && (terrainSessionNow.sessionId || isTerrainModuleActive)) {
          ...
          console.log(
            ' Chat: Including screenshot for terrain agent:',
            !!currentMapCtx?.imagery_base64
          );
        ...
        if (selectedModule === 'comparison') {
          ...
          console.log(
            ' Chat: Including screenshot for comparison agent:',
            !!currentMapCtx?.imagery_base64
          );
        ...
        if (selectedModule === 'mobility' && mobilityPinCoords) {
          ...
          console.log(
            ' Chat: Including screenshot for mobility agent:',
            !!currentMapCtx?.imagery_base64
          );
        ...
        if (selectedModule === 'building_damage') {
          ...
          console.log(
            ' Chat: Including screenshot for building damage agent:',
            !!currentMapCtx?.imagery_base64
          );
          ...
          try {
            ...
            const damageAnalysis =
              ...
              response?.result?.summary ||
              response?.message ||
              'Building damage analysis complete.';
            return {
        ...
        if (selectedModule === 'extreme_weather') {
          ...
          console.log(' Chat: Coordinates:', { lat, lng });
          console.log(
            ' Chat: Including screenshot for extreme weather agent:',
            !!currentMapCtx?.imagery_base64
          );
          ...
          try {
            ...
            console.log(
              ' Chat: response.result?.analysis length:',
              response.result?.analysis?.length
            );
            ...
            const climateAnalysis =
              ...
              response.message ||
              response.content ||
              'Extreme weather analysis complete.';
            return {
  ...
  const privateSearchMutation = useMutation({
    mutationFn: async ({ query, collection_id }: { query: string; collection_id?: string }) => {
      try {
        ...
        throw new Error(
          'Failed to search VEDA datasets. Please check your configuration and try again.'
        );
      }
    },
    onSuccess: (responseData) => {
    ...
    onError: (error) => {
      setMessages((prev) => [
        ...prev,
        {
          ...
          content:
            error?.message ||
            'Sorry, I encountered an error with the private search. Please try again.',
          timestamp: new Date(),
        },
      ]);
    },
  ...
  const handleSendMessage = () => {
    ...
    console.log(' Chat: Sending message via main chat endpoint:', {
      inputValue,
