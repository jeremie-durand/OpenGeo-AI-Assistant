...
class DeployedMCPTester:
    ...
    def test_read_resource(self, uri="earth://stac/sentinel-2"):
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/resources/read", json={"uri": uri}
            )
            response.raise_for_status()
    ...
    def test_api_documentation(self):
        ...
        try:
            ...
            print("   Open this URL in your browser for interactive docs")
    ...
    def test_response_times(self):
        ...
        try:
            ...
            if health_time > 1000:
                print(
                    f"   [WARN]  Warning: Health check is slow (>{health_time:.0f}ms)"
                )
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print(
            "  python test_deployed_mcp.py https://earth-copilot-mcp.azurecontainerapps.io"
        )
    ...
    if not tester.test_health():
        ...
        print(
            "   4. View logs: az containerapp logs show --name <app-name> --resource-group <rg-name>"
        )
        sys.exit(1)
